...
import { CODE_TEXT_HEIGHT, REPLAY_CODE_ROI } from "../../app/features/cv/core/detectors/scoreboard-replay/rois";
import type { AtlasMeta } from "../../app/features/cv/core/glyphs";
import { CV_ASSETS_DIR } from "../../app/features/cv/node/assets-dir";
import { normalizeFrame, type Roi, toMat } from "../../app/features/cv/core/image";
...
const SOURCES: Source[] = [
  ...
  },
  {
    ...
    names: ["Mongering", "y0s", "fuzzy", "kera", "sigma", "Reefslider", "Cucumber", "tomato"],
  ...
  },
  {
    ...
    names: ["Bocchi", "have faith", "", "Florescent", "Elis", "Jrod_14", "GOLD SHIP", ""],
...
async function loadTagBand(framePath: string): Promise<Mat> {
  ...
  const outerView = rgb.roi(
    new cv.Rect(TAG_NAME_OUTER.x, TAG_NAME_OUTER.y, TAG_NAME_OUTER.w, TAG_NAME_OUTER.h),
  );
  ...
  const innerView = rotated.roi(
    new cv.Rect(TAG_NAME_INNER.x, TAG_NAME_INNER.y, TAG_NAME_INNER.w, TAG_NAME_INNER.h),
  );
...
function columnRuns(
  ...
  for (const hint of mergeHints) {
    const i = runs.findIndex(
      (r, idx) => idx + 1 < runs.length && r.x1 <= hint && runs[idx + 1]!.x0 >= hint,
    );
    if (i >= 0) {
      runs = [...runs.slice(0, i), { x0: runs[i]!.x0, x1: runs[i + 1]!.x1 }, ...runs.slice(i + 2)];
    }
  }
  return runs.flatMap((run) => {
    const cuts = splitHints.filter((h) => h > run.x0 + 1 && h < run.x1 - 1).sort((a, b) => a - b);
...
function cropGlyph(run: { x0: number; x1: number }, roi: Roi, threshold = BIN_THRESHOLD): Mat {
...
function writeAtlas(
  ...
  const entries = collected instanceof Map ? [...collected.entries()] : collected;
  ...
  const spacing = 2;
  const totalW = glyphs.reduce((acc, [, m]) => acc + m.cols, 0) + spacing * (glyphs.length + 1);
  ...
  for (const [char, m] of glyphs) {
    ...
    m.copyTo(view);
    view.delete();
    meta.glyphs.push({ char, x, y: spacing, w: m.cols, h: m.rows, source: "fixture" });
    x += m.cols + spacing;
  }
  ...
  atlas.delete();
  rgba.delete();
  console.log(`${name}: ${glyphs.length} glyphs -> ${OUT_DIR}/${name}.{png,json}`);
}
...
for (const source of SOURCES) {
  console.log(`harvesting ${source.frame}`);
  ...
  if (source.header) {
    ...
    harvest(lobby.roi, lobby.label, lobbyGlyphs, lobby.splitHints, lobby.threshold);
    ...
    harvest(stage.roi, stage.label, stageGlyphs, stage.splitHints, stage.threshold);
...
const DEATH_TAG_SOURCES: { frame: string; label: string; mergeHints?: number[] }[] = [
  {
...
const DEATH_JA_SOURCES: {
...
}[] = [
  {
    ...
    lines: [
      ...
      { roi: { x: 825, y: 358, w: 275, h: 58 }, label: "スクイックリンα で", mergeHints: [917] },
