...
invariant(discordIds.includes(","), "discordIds must be a comma separated list of discord ids");
...
const userStm = sql.prepare(/* sql */ `select "id" from "User" where "discordId" = @discordId`);
