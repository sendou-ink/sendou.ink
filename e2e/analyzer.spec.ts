...
import { impersonate, isNotVisible, navigate, seed, selectWeapon } from "~/utils/playwright";
import { ANALYZER_URL } from "~/utils/urls";

test.describe("Build Analyzer", () => {
	test("analyzes a build and links to new build page with same abilities", async ({ page }) => {
		await seed(page);
		...
		await expect(page.getByTestId("weapon-combobox-input")).toHaveValue("Luna Blaster");
	...
	test("compares builds", async ({ page }) => {
		...
		const swimSpeedAbilityButtonLocator = page.getByTestId("SSU-ability-button");
		...
		await expect(page.getByTestId("ap-compare-1").first()).toContainText("10AP");
		await expect(page.getByTestId("ap-compare-2").first()).toContainText("13AP");
