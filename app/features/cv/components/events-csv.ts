...
function formatMinimapPlayers(data: MinimapData): string {
  const fmt = (
    label: string,
    p: { name: string | null; weaponId: number | null; abilities: (string | null)[] },
  ) =>
...
function eventCells(event: CsvEvent): Cell[] {
  const base: Cell[] = [
    ...
    event.detectedAt === undefined ? "" : new Date(event.detectedAt).toISOString(),
  ...
  switch (event.type) {
    ...
    case MAP_START_EVENT_TYPE: {
      ...
      return [...base, "", modeLabel(d.mode), stageLabel(d.stage), "", "", "", "", "", "", "", "", ""];
    }
...
function eventsToCsv(events: CsvEvent[]): string {
  ...
  for (const event of events) lines.push(eventCells(event).map(csvCell).join(","));
  return `${lines.join("\r\n")}\r\n`;
}
