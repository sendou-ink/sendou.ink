...
import { listVideoInputs, openVirtualCamera, startSampler } from "../capture/sampler";
...
const INGESTABLE_TYPES = [MAP_START_EVENT_TYPE, DEATH_EVENT_TYPE, ...SCOREBOARD_EVENT_TYPES];
...
export function LivePage({
  ...
  const latestParseRef = useRef<{ type: string; data: FixtureData } | null>(null);
  ...
  const refreshFeed = useCallback(() => {
    void listEvents().then((events) =>
      setFeed(events.sort((a, b) => b.detectedAt - a.detectedAt || (b.id ?? 0) - (a.id ?? 0))),
    );
  }, []);
  ...
  const send = useCallback(
    async (include: (batch: StoredEvent[]) => boolean, { manual = false } = {}) => {
      ...
      try {
        ...
        if (manual && sentBatches + failedBatches === 0) {
          setSendouError("nothing to send — no complete match (ending in a scoreboard) selected");
        }
      } finally {
          ...
          for (const event of result.events as DetectedEvent<FixtureData>[]) {
            ...
            if (action.action === "added" || action.action === "replaced") {
              ...
              void (async () => {
                ...
                const thumbnail = result.frame ? await thumbnailFromBlob(result.frame) : undefined;
                const id = await saveEvent(event, thumbnail, result.frame);
                storedIdsRef.current.set(event, id);
                if (liveSendRef.current && INGESTABLE_TYPES.includes(event.type)) {
                  if (SCOREBOARD_EVENT_TYPES.includes(event.type)) {
                    // a scoreboard closes its match batch — send it
                    refreshFeed();
                    await send((batch) => batchContaining(id)(batch) && unsentBatches(batch));
                  } else {
  ...
  return (
    <div>
      <div className="controls">
        {!running ? (
          <button onClick={() => void start()}>Start capture</button>
        ) : (
        ...
        <button
          disabled={!running}
          onClick={() => void saveFixture(videoRef.current!, latestParseRef.current)}
        >
        ...
        <button
          ...
          onClick={() =>
            downloadEventsCsv(
              ...
              [...feed].sort((a, b) => a.detectedAt - b.detectedAt || (a.id ?? 0) - (b.id ?? 0)),
            )
          }
        >
          Download CSV
      ...
      {SENDOU_UPLOAD_ENABLED && (
        <div className="controls">
          ...
          {liveSend && <span className="status watching">sending matches live</span>}
        </div>
      )}
        ...
        <div className="feed">
          ...
          {feed.map((e) => (
            <EventCard
              ...
              getFrame={e.hasFrame && e.id !== undefined ? () => loadEventFrame(e.id!) : undefined}
              send={e.send}
              onSend={
                SENDOU_UPLOAD_ENABLED &&
                sendouUser &&
