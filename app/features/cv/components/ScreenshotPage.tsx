...
import { lobbyLabel, mainWeaponLabel, modeLabel, stageLabel, weaponLabel } from "./labels";
...
function RoiCrop(props: { frame: HTMLCanvasElement; roi: Roi; scale?: number }) {
  ...
  useEffect(() => {
    ...
    ctx.drawImage(frame, roi.x, roi.y, roi.w, roi.h, 0, 0, canvas.width, canvas.height);
...
function replayRows(winnerSide: string): RowRois[] {
  const panels = winnerSide === "right" ? [replay.PANEL_DX, 0] : [0, replay.PANEL_DX];
  return panels.flatMap((dx) =>
    replay.ROW_CENTERS.map((cy) => ({
      ...
      stats: [replay.statRoi(cy, dx, 0), replay.statRoi(cy, dx, 1), replay.statRoi(cy, dx, 2)] as [
        Roi,
        Roi,
        Roi,
      ],
...
function drawOverlay(ctx: CanvasRenderingContext2D, detector: string) {
  ...
  if (detector === "death") {
    ...
    for (let row = 0; row < death.ABILITY_ROWS; row++) {
      ...
      for (const slot of [0, 1, 2]) rect(death.abilitySubRoi(row, slot), "#e879f9");
    }
  ...
  if (detector === "scoreboard-own") {
    ...
    for (const roi of [...own.GATE_PANEL_PROBES, ...own.GATE_TITLE_TEXT_PROBES]) {
      rect(roi, "#facc15");
    }
  ...
  if (detector === "minimap") {
    for (const card of minimap.CARD_LAYOUTS) {
      ...
      for (const [cx, cy] of card.badges) rect(minimap.badgeRoi(cx, cy), "#60a5fa");
    ...
    for (const cy of minimap.ENEMY_ROW_CYS) {
      ...
      for (const cx of minimap.ENEMY_BADGE_XS) rect(minimap.badgeRoi(cx, cy), "#60a5fa");
  ...
  if (detector === "scoreboard-replay") {
    for (const dx of replay.PANEL_XS) {
      for (const cy of replay.ROW_CENTERS) {
        ...
        for (const i of [0, 1, 2] as const) rect(replay.statRoi(cy, dx, i), "#e879f9");
...
export function ScreenshotPage() {
  ...
  const analyze = useCallback(async (file: File | Blob) => {
    ...
    try {
      ...
      norm.getContext("2d")!.drawImage(bitmap, 0, 0, CANONICAL_WIDTH, CANONICAL_HEIGHT);
      setFrame(norm);
  ...
  return (
    <div>
      ...
      <div className="screenshot-frame" style={{ display: frame ? "block" : "none" }}>
        <canvas ref={displayRef} />
      </div>

      {frame && !busy && (
        <p>
          <button onClick={() => downloadExpectedJson(event?.data ?? null, event?.type)}>
      ...
      {Object.values(results).map((result) => (
        <p key={result.detector}>
          {result.detector} gate: <b>{result.gate.pass ? "fired" : "no fire"}</b> (score{" "}
          {result.gate.score.toFixed(3)})
          {result.events[0] && result.detector === "death" && (
            <>
              {" · "}confidence {((result.events[0].confidence ?? 0) * 100).toFixed(1)}%{" · "}
              {(() => {
          ...
          {result.events[0] && result.detector === "minimap" && (
            <>
              ...
              {(() => {
                ...
                const players = data.teammates.map((p) => p.name ?? "?").join(", ");
          ...
          {result.events[0] &&
            ...
            result.detector !== "minimap" && (
              <>
                {" · "}confidence {((result.events[0].confidence ?? 0) * 100).toFixed(1)}% · scores{" "}
                {JSON.stringify((result.events[0].data as CardData).scores)}
                {" · "}
                {(() => {
                  const data = result.events[0].data as CardData;
                  return [lobbyLabel(data.lobby), modeLabel(data.mode), stageLabel(data.stage)]
                    .map((v) => v ?? "?")
                    .join(" · ");
                })()}
              </>
      ...
      {frame && event && isReplay && (
        <p>
          ...
          <span className="score">(raw: {String(event.debug?.codeRaw ?? "")})</span>
      ...
      {frame && event && isDeath && (
        <p>
          {(() => {
            ...
            return (
              <>
                weapon <b>{weaponLabel(data.weaponType, data.weaponId) ?? "?"}</b>{" "}
                <span className="score">(raw: {String(event.debug?.weaponRaw ?? "")})</span>
                {" · "}name <b>{data.name ?? "?"}</b>{" "}
                <span className="score">(raw: {String(event.debug?.nameRaw ?? "")})</span>
                {" · "}abilities {data.abilities.map((row) => row.join(" ")).join(" | ")}
                <br />
          ...
          {(() => {
            ...
            return (
              <>
                ...
                <span className="score">(raw: {String(event.debug?.modeReading ?? "")})</span>
                {" · "}stage <b>{stageLabel(data.stage) ?? "?"}</b>{" "}
                <span className="score">(raw: {String(event.debug?.stageReading ?? "")})</span>
                <br />
                <RoiCrop frame={frame} roi={mapStart.MODE_BLOCK_ROI} scale={0.75} />{" "}
          ...
          {(() => {
            ...
            return (
              <>
                ...
                <span className="score">(raw: {String(event.debug?.weaponReading ?? "")})</span>
                {" · "}abilities {data.abilities.map((row) => row.join(" ")).join(" | ")}
                <br />
                    ...
                    <div className="candidates">
                      {([0, 1, 2] as const).map((s) => (
                        <span className="cand" key={s}>
                          ...
                          <span className="score">{dbg?.statScores[s].toFixed(2)}</span>
                        </span>
                      ))}
                    </div>
                  </td>
