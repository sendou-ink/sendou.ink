    ...
    <div className="card">
      <div className="meta">
        ...
        {(data.mode !== null || data.stage !== null) && (
          <span>
            {[lobbyLabel(data.lobby), modeLabel(data.mode), stageLabel(data.stage)]
              .filter(Boolean)
              .join(" · ")}
          </span>
        )}
        ...
        {getFrame && (
          <button
            onClick={() =>
              void getFrame().then((f) => f && saveFixtureFromEvent(f, data, eventType))
            }
          >
            Save fixture
          </button>
      ...
      <div className="teams">
        <div className="team win">
          ...
          <PlayerRows players={data.players.slice(0, 4)} offset={0} abilities={props.abilities} />
        </div>
        <div className="team lose">
          <h3>{teamHeading("Defeat", data, 1)}</h3>
          <PlayerRows players={data.players.slice(4, 8)} offset={4} abilities={props.abilities} />
        </div>
      </div>
    </div>
  );
