...
export function VodPage({
  ...
  const abilityMap = useMemo(() => connectAbilities(matches.map((m) => m.event)), [matches]);
  ...
  const uploadResults = useCallback(async () => {
    ...
    setResultsSend({ state: "sending", sent: 0, total: countIngestBatches(events) });
      ...
      try {
                ...
                (async () => {
                  ...
                  const next =
                    action.action === "replaced"
                      ? matchesRef.current.filter((m) => m.event !== action.replaced)
        ...
        for await (const { frame, t } of vod.frames) {
          ...
          if (now - lastUiUpdate >= 250) {
            ...
            setProgress({ t, duration: vod.duration, rate: elapsed > 0 ? t / elapsed : 0 });
          }
          if (!pool.hasIdle()) {
  ...
  return (
    <div>
      ...
      <div className="controls">
        {showVodView && (
          <>
            <button onClick={backToList}>← All VoDs</button>
            <span
            ...
            >
              ...
              {fileName && ` · ${fileName}`}
              {method && ` · ${method}`}
              {gateScore !== null && status === "scanning" && ` · gate ${gateScore.toFixed(2)}`}
            </span>
            {progress && (
              <span className="score">
                ...
                {progress.rate > 0 && ` · ${progress.rate.toFixed(0)}× realtime`}
              </span>
            )}
            {matches.length > 0 && (
              <span className="score">
      ...
      {!showVodView && (
        <div className="vod-list">
          {vods.length === 0 && (
            <p className="score">No saved VoDs yet — scan one and it will show up here.</p>
          )}
          {vods.map((vod) => (
            <div key={vod.name} className="vod-item">
              <span className="name">{vod.name}</span>
              <span className="score">
                {vod.eventCount} event{vod.eventCount === 1 ? "" : "s"} · {formatTime(vod.duration)}{" "}
                · {new Date(vod.savedAt).toLocaleString()}
              </span>
              <button onClick={() => void openStored(vod.name)}>Open</button>
              <button onClick={() => void removeVod(vod.name)}>Delete</button>
            </div>
          ))}
        </div>
      )}
      ...
      >
        <div style={{ display: source === "scan" ? undefined : "none" }}>
          ...
          <video
            ...
            playsInline
            controls
            style={{ display: fileName && status !== "scanning" ? "block" : "none" }}
          />
        </div>
        <div className="feed">
          {matches.length === 0 && (
...
function drawPreview(canvas: HTMLCanvasElement | null, frame: ImageBitmap | VideoFrame): void {
  if (!canvas) return;
