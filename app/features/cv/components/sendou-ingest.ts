...
export async function sendBatches({
  ...
  const allBatches = buildIngestBatches(events.filter((e) => e.id !== undefined));
...
async function postIngestBatch(batch: Array<DetectedEvent & { detectedAt?: number }>) {
  const res = await fetch(INGEST_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ events: batch.slice(0, MAX_EVENTS_PER_REQUEST).map(payloadEvent) }),
  });
  if (!res.ok) {
    throw new Error(res.status === 401 ? "not logged in to sendou.ink" : await errorText(res));
  }
}
...
async function clearOrphanedQueued(
  ...
  const lastBoundaryT = Math.max(...allBatches.map((b) => b.at(-1)!.t), Number.NEGATIVE_INFINITY);
...
export function unsentBatches(batch: StoredEvent[]): boolean {
  return !batch.some((e) => e.send?.state === "sent" || e.send?.state === "sending");
}

function payloadEvent(event: DetectedEvent & { detectedAt?: number }) {
  const recordedAt =
    event.type === SCOREBOARD_REPLAY_EVENT_TYPE
      ? replayRecordedAt(event.data as { timestamp: string | null }, event.detectedAt)
      : null;
  return {
    type: event.type,
    t: event.t,
...
function replayRecordedAt(
  ...
  return data.timestamp ? parseReplayTimestamp(data.timestamp, { now: detectedAt }) : null;
}
