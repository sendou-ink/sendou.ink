...
import { MAP_START_EVENT_TYPE, type MapStartData } from "../core/detectors/map-start/index";
...
export type CardData = ScoreboardData &
  Partial<Pick<ScoreboardReplayData, "timestamp" | "replayCode" | "matchScores">>;

/** Any detector's event payload that can prefill a fixture. */
export type FixtureData = CardData | DeathData | MapStartData | ScoreboardOwnData | MinimapData;
...
function buildExpectedJson(data: FixtureData | null, eventType = "Scoreboard"): string {
  if (!data) {
    ...
    return `${JSON.stringify(
      {
        ...
        data: {
          ...
          ...(own.stage !== null && { stage: own.stage, stageLabel: stageLabel(own.stage) }),
          ...(own.weaponId !== null && {
  ...
  return `${JSON.stringify(
    {
      ...
      data: {
        ...
        ...(card.stage !== null && { stage: card.stage, stageLabel: stageLabel(card.stage) }),
        ...(card.timestamp != null && { timestamp: card.timestamp }),
        ...(card.replayCode != null && { replayCode: card.replayCode }),
        scores: card.scores,
        ...(card.matchScores !== undefined && { matchScores: card.matchScores }),
 ...
 */
export function downloadExpectedJson(data: FixtureData | null, eventType?: string): void {
  download(
    "expected.json",
    new Blob([buildExpectedJson(data, eventType)], { type: "application/json" }),
  );
}
 ...
 */
export function saveFixtureFromEvent(frame: Blob, data: FixtureData, eventType: string): void {
...
export async function saveFixture(
  ...
  download(
    "expected.json",
    new Blob([buildExpectedJson(latest?.data ?? null, latest?.type)], { type: "application/json" }),
  );
}
