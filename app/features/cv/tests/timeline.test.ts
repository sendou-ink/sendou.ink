...
function event(t: number, confidence: number, type = "Scoreboard"): DetectedEvent {
...
function replay(
  t: number,
  confidence: number,
  data: Partial<ScoreboardData> & { timestamp?: string | null; replayCode?: string | null },
): DetectedEvent {
  return {
...
test("different recording timestamps split, equal ones merge", () => {
  ...
  assert.equal(tl.push(replay(104, 0.8, { timestamp: "3/7/2026 21:22" })).action, "added");
  assert.equal(tl.push(replay(108, 0.7, { timestamp: "3/7/2026 21:15" })).action, "merged");
});
...
test("replay codes tolerate misread glyphs but split on real differences", () => {
  ...
  assert.equal(tl.push(replay(104, 0.7, { replayCode: "R797-U51Y-945W-G4JL" })).action, "merged");
  assert.equal(tl.push(replay(108, 0.8, { replayCode: "RVRM-XXEL-0573-Q4SV" })).action, "added");
});

test("disjoint paint totals split when stage and code are unreadable", () => {
  const tl = new TimelineBuilder({ minConfidence: 0 });
  tl.push(replay(100, 0.8, { players: players([327, 408, 270, 354, 381, 613, 456, 287]) }));
...
test("too few readable paints never split", () => {
  const tl = new TimelineBuilder({ minConfidence: 0 });
  tl.push(replay(100, 0.8, { players: players([327, 408, 270, null, null, null, null, null]) }));
