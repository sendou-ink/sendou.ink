...
import { createMinimapDetector, type MinimapData } from "../core/detectors/minimap/index";
...
for (const fixture of fixtures) {
  test(`minimap/${fixture.name}`, async (t) => {
    ...
    await t.test("spectator", { skip: expected.spectator === undefined }, () => {
      assert.equal(event.data.spectator, expected.spectator);
    });

    await t.test("stage", { skip: expected.stage === undefined || skip(fixture, "stage") }, () => {
      assert.equal(
        event.data.stage,
        expected.stage,
    ...
    await t.test("teammate count", { skip: expected.teammates === undefined }, () => {
      assert.equal(event.data.teammates.length, expected.teammates?.length);
    });
    await t.test("enemy count", { skip: expected.enemies === undefined }, () => {
    ...
    for (const [i, want] of (expected.teammates ?? []).entries()) {
      await t.test(
        ...
        () => {
          ...
          assert.ok(got, `teammate ${i} missing (got ${event.data.teammates.length})`);
          const cardDebug = JSON.stringify((event.debug?.cards as unknown[])?.[i]);
          ...
          if (
            ...
            assert.deepEqual(got.abilities, want.abilities, `abilities (debug: ${cardDebug})`);
          }
        },
      );
    }
    ...
    for (const [i, want] of (expected.enemies ?? []).entries()) {
      await t.test(`enemy ${i}`, { skip: skip(fixture, `enemies.${i}`) }, () => {
        const got = event.data.enemies[i];
        assert.ok(got, `enemy row ${i} missing (got ${event.data.enemies.length})`);
        const rowDebug = JSON.stringify((event.debug?.enemies as unknown[])?.[i]);
...
const otherPositives = [
  ...
  ...loadFixtures("scoreboard-replay").filter((f) => f.expected.event === "ScoreboardReplay"),
  ...loadFixtures("scoreboard-own").filter((f) => f.expected.event === "ScoreboardOwn"),
...
for (const fixture of otherPositives) {
  test(`minimap gate stays quiet on ${fixture.dir.split("/").slice(-2).join("/")}`, async () => {
    ...
    assert.equal(gate.pass, false, `minimap gate fired (score=${gate.score.toFixed(3)})`);
  });
}
...
for (const fixture of fixtures.filter((f) => f.expected.event === "Minimap")) {
  test(`other gates stay quiet on minimap/${fixture.name}`, async () => {
    for (const other of otherDetectors) {
      ...
      assert.equal(gate.pass, false, `${other.id} gate fired (score=${gate.score.toFixed(3)})`);
    }
  });
}
