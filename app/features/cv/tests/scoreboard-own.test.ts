...
for (const fixture of fixtures) {
  test(`scoreboard-own/${fixture.name}`, async (t) => {
    ...
    for (const field of ["lobby", "mode", "stage"] as const) {
      await t.test(field, { skip: expected[field] === undefined || skip(fixture, field) }, () => {
        assert.equal(
          event.data[field],
          expected[field],
...
for (const fixture of otherPositives) {
  test(`scoreboard-own gate stays quiet on ${fixture.dir.split("/").slice(-2).join("/")}`, async () => {
    ...
    assert.equal(gate.pass, false, `scoreboard-own gate fired (score=${gate.score.toFixed(3)})`);
  });
}
...
for (const fixture of fixtures.filter((f) => f.expected.event === "ScoreboardOwn")) {
