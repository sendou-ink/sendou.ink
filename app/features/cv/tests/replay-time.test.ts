...
function local(year: number, month: number, day: number, h: number, min: number): number {
  return new Date(year, month - 1, day, h, min).getTime();
}

test("day-first locale reads D/M/Y", () => {
  assert.equal(
    parseReplayTimestamp("3/7/2026 22:28", { locale: "fi-FI", now: local(2026, 7, 4, 10, 0) }),
...
test("a 24h hour implies day-first even under a month-first browser locale", () => {
  ...
  assert.equal(
    parseReplayTimestamp("3.7.2026 21:15", { locale: "en-US", now: local(2026, 7, 5, 12, 0) }),
    local(2026, 7, 3, 21, 15),
  );
  // midnight hours only exist on a 24h clock too
  assert.equal(
    parseReplayTimestamp("3/7/2026 0:45", { locale: "en-US", now: local(2026, 7, 4, 10, 0) }),
    local(2026, 7, 3, 0, 45),
  );
});
...
test("month-first locale reads M/D/Y when the hour is clock-ambiguous", () => {
  ...
  assert.equal(
    parseReplayTimestamp("3/7/2026 9:15", { locale: "en-US", now: local(2026, 5, 10, 12, 0) }),
    local(2026, 3, 7, 9, 15),
  );
});
...
test("recency flips a locale misread whose swap lands near now", () => {
  ...
  assert.equal(
    parseReplayTimestamp("3/7/2026 9:15", { locale: "en-US", now: local(2026, 7, 4, 12, 0) }),
    local(2026, 7, 3, 9, 15),
  );
});
...
test("recency leaves a recent locale reading alone", () => {
  assert.equal(
    parseReplayTimestamp("3/7/2026 9:15", { locale: "en-US", now: local(2026, 3, 8, 12, 0) }),
    local(2026, 3, 7, 9, 15),
  );
});
...
test("an old replay keeps the locale order when neither reading is recent", () => {
  assert.equal(
    parseReplayTimestamp("5.1.2026 9:15", { locale: "fi-FI", now: local(2026, 7, 15, 12, 0) }),
    local(2026, 1, 5, 9, 15),
  );
});
...
test("year-first format is month-first, regardless of clock or recency", () => {
  assert.equal(
    parseReplayTimestamp("2026/3/7 22:28", { locale: "ja-JP", now: local(2026, 7, 4, 12, 0) }),
    local(2026, 3, 7, 22, 28),
  );
});
...
test("two-digit year lands in the 2000s", () => {
  assert.equal(
    parseReplayTimestamp("3/7/26 22:28", { locale: "fi-FI", now: local(2026, 7, 4, 10, 0) }),
    local(2026, 7, 3, 22, 28),
  );
});

test("invalid dates and times are rejected", () => {
  assert.equal(parseReplayTimestamp("31/2/2026 10:00", { locale: "fi-FI" }), null);
