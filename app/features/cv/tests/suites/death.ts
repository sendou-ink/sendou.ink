...
import { createDeathDetector, type DeathData } from "../../core/detectors/death/index";
...
export async function runDeathSuite(shard: number, shardCount: number): Promise<void> {
  await loadOpenCV();
  ...
  for (const fixture of mine(fixtures)) {
    test(`death/${fixture.name}`, async (t) => {
      ...
      await t.test(
        ...
        () => {
          assert.equal(
            event.data.weapon,
            expected.weapon,
            `weapon mismatch (raw: "${event.debug?.weaponRaw}", score=${Number(event.debug?.weaponScore ?? 0).toFixed(3)})`,
          );
      ...
      for (const [row, wantRow] of (expected.abilities ?? []).entries()) {
        await t.test(`abilities row ${row}`, { skip: skip(fixture, `abilities.${row}`) }, () => {
          assert.deepEqual(
      ...
      await t.test("name", { skip: expected.name === undefined || skip(fixture, "name") }, () => {
        assert.equal(
          event.data.name,
          expected.name,
  ...
  for (const fixture of mine([...scoreboardPositives, ...replayPositives])) {
    test(`death gate stays quiet on ${fixture.dir.split("/").slice(-2).join("/")}`, async () => {
      ...
      assert.equal(gate.pass, false, `death gate fired (score=${gate.score.toFixed(3)})`);
    });
  }
  ...
  const crossFixtures = mine(fixtures.filter((f) => f.expected.event === "Death"));
