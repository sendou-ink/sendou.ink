/**
 * Golden-file suite for the ScoreboardReplayDetector over every fixture in
 * scoreboard-replay/, mirroring tests/scoreboard.test.ts, plus the replay
 * extras (timestamp, replay code, match scores) and a cross-negative sweep:
 * the replay gate must stay quiet on every live-scoreboard positive.
 *
 * Replay parses are expensive (eight 68px weapon rows plus the Rowdy-face
 * code line), so the suite is sharded across processes:
 * tests/scoreboard-replay.<n>.test.ts each run one shard. Every fixture
 * group is dealt round-robin, so `node --test tests/scoreboard-replay.*.test.ts`
...
export async function runScoreboardReplaySuite(shard: number, shardCount: number): Promise<void> {
  await loadOpenCV();
  const detector = createScoreboardReplayDetector(await loadScoreboardResources());
  const fixtures = loadFixtures("scoreboard-replay");
  const mine = <T>(items: T[]): T[] => items.filter((_, i) => i % shardCount === shard);

  if (shard === 0) {
    test("replay fixtures exist", () => {
      assert.ok(fixtures.length > 0, "no fixtures found under scoreboard-replay/");
    });
  }

  for (const fixture of mine(fixtures)) {
    test(`scoreboard-replay/${fixture.name}`, async (t) => {
      const { gate, events } = await runDetectorOnFixture(detector, fixture);
      const expectPositive = fixture.expected.event === "ScoreboardReplay";
      ...
      await t.test(
        "matchScores",
        { skip: expected.matchScores === undefined || skip(fixture, "matchScores") },
        () => {
      ...
      await t.test(
        ...
        () => {
          const dbg = event.debug?.header as
            ...
            | undefined;
          assert.deepEqual(
            { lobby: event.data.lobby, mode: event.data.mode, stage: event.data.stage },
            {
      ...
      await t.test(
        "replayCode",
        { skip: expected.replayCode === undefined || skip(fixture, "replayCode") },
        () => {
          assert.equal(
      ...
      for (const [i, want] of players.entries()) {
        ...
        await t.test(
          `row ${i} weapon`,
          { skip: want.weaponId === undefined || skip(fixture, `players.${i}.weaponId`) },
          () => {
            const top = dbg?.weapon?.top.map((c) => `${c.id}:${c.score.toFixed(3)}`).join(" ");
            assert.equal(got.weaponId, want.weaponId, `weapon mismatch (candidates: ${top})`);
          },
        );
  ...
  for (const fixture of mine(
    ...
    test(`replay gate stays quiet on scoreboard/${fixture.name}`, async () => {
      ...
      assert.equal(gate.pass, false, `replay gate fired (score=${gate.score.toFixed(3)})`);
    });
  }
