...
import { createSearchParams, Link, useLoaderData, useSearchParams } from "@remix-run/react";
...
export const meta: MetaFunction = (args) => {
  ...
  return metaTags({
    ...
    description: `${data.events.length} events on sendou.ink happening during week ${data.displayedWeek
...
function ICalLink() {
  ...
  const filteredTags = (searchParams
    .get("tags")
    ?.split(",")
    .filter((tag) => CALENDAR_EVENT.TAGS.includes(tag as CalendarEventTag)) ??
    [])
  ...
  if (filteredTags.length > 0)
    params.append("tags", filteredTags);
  if (onlyTournaments)
    params.append("tournaments", "true");

  let icalURL = "https://sendou.ink/calendar.ics" + ((params.size > 0) ? `?${params.toString()}` : "");
