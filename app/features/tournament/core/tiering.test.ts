...
describe("tierNumberToName", () => {
	test("converts all tier numbers correctly", () => {
	...
	test("throws for invalid tier numbers", () => {
...
describe("calculateAdjustedScore", () => {
	test("returns raw score when at X-tier threshold (no bonus)", () => {
		expect(calculateAdjustedScore(32, 50)).toBe(32);
	});

	test("returns raw score when below MIN_TEAMS_FOR_TIERING", () => {
		expect(calculateAdjustedScore(20, MIN_TEAMS_FOR_TIERING)).toBe(20);
	});

	test("applies size bonus for lower skill tournaments with many teams", () => {
		const rawScore = 20;
		const teamCount = 50;
	...
	test("applies larger bonus at lower skill levels", () => {
		const teamCount = 50;
	...
	test("applies larger bonus for more teams", () => {
		const rawScore = 15;
	...
	test("calculates correct bonus for specific case", () => {
		const rawScore = 0;
		const teamCount = 18;
...
describe("calculateTournamentTierFromTeams", () => {
	test("returns null tier for tournaments below MIN_TEAMS_FOR_TIERING", () => {
	...
	test("returns null tier when all teams have null ordinals", () => {
	...
	test("returns null tier for empty teams array with sufficient count", () => {
		const result = calculateTournamentTierFromTeams([], 10);

		expect(result.tierNumber).toBe(null);
	});

	test("calculates tier from top 8 teams by ordinal", () => {
		const teams = [
			{ avgOrdinal: 35 },
			{ avgOrdinal: 34 },
			{ avgOrdinal: 33 },
	...
	test("ignores teams with null ordinals when calculating", () => {
		const teams = [
			{ avgOrdinal: 30 },
			{ avgOrdinal: null },
			{ avgOrdinal: 28 },
	...
	test("applies size bonus correctly", () => {
	...
	test("uses fewer than 8 teams if not enough available", () => {
...
describe("TIER_THRESHOLDS matches TIER_TO_NUMBER", () => {
	test("all tiers in TIER_THRESHOLDS have corresponding number", () => {
...
describe("calculateTentativeTier", () => {
	...
	test("returns the single value for history with one element", () => {
	...
	test("rounds toward lower tier (higher number) for even count", () => {
		expect(calculateTentativeTier([4, 5])).toBe(5);
		expect(calculateTentativeTier([3, 4])).toBe(4);
	});

	test("returns median for unsorted input", () => {
...
describe("updateTierHistory", () => {
	test("creates new history from null", () => {
