...
function cleanTranslations(
	...
	for (const [key, value] of Object.entries(translations)) {
		...
		if (typeof value === 'string') {
			...
			const englishNested =
				typeof englishValue === 'object' && englishValue !== null ? englishValue : {};
...
async function processTranslationFiles(messagesDir: string): Promise<void> {
	try {
		...
		const jsonFiles = files.filter((file) => file.endsWith('.json') && file !== 'en.json');
		...
		for (const file of jsonFiles) {
			...
			console.log(
				`${locale}: ${originalKeys - cleanedKeys} keys removed, ${cleanedKeys} keys remaining`
			);
		}
