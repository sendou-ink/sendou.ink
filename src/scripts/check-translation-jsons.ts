...
const REPO_TRANSLATIONS_INFO_URL = 'https://github.com/sendou-ink/sendou.ink#translations';
...
function otherLanguageTranslationPath(code?: string, fileName?: string) {
	return path.join(
		...[__dirname, '..', 'locales', code, fileName].filter((val): val is string => !!val)
	);
}
