...
async function main() {
	logger.info('Starting to fix tournamentTeamId in TournamentMatchGameResultParticipant');
	...
	const result: Array<{ tournamentId: number } & Pick<TournamentSummary, 'setResults'>> = [];
	...
	await db
		...
		.set((eb) => ({
			tournamentTeamId: eb
				...
				.innerJoin('TournamentTeam', 'TournamentTeamMember.tournamentTeamId', 'TournamentTeam.id')
				...
				.whereRef('TournamentTeamMember.userId', '=', 'TournamentMatchGameResultParticipant.userId')
				.whereRef(
					'TournamentTeam.tournamentId',
					'=',
					eb
						.selectFrom('TournamentMatchGameResult')
						.innerJoin('TournamentMatch', 'TournamentMatchGameResult.matchId', 'TournamentMatch.id')
						.innerJoin('TournamentStage', 'TournamentStage.id', 'TournamentMatch.stageId')
...
async function wipeEmptyResults() {
	...
	logger.info(`Wiped ${numDeletedRows} empty results from TournamentResult table.`);
}
