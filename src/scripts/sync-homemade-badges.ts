...
async function main() {
	...
	for (const existingBadge of await homemadeBadgesInDb()) {
		const badge = homemadeBadges[existingBadge.code as keyof typeof homemadeBadges];
	...
	for (const [fileName, badge] of Object.entries(homemadeBadges)) {
		const existing = homemadeAfterUpdates.find((existingBadge) => fileName === existingBadge.code);
		...
		const author = await findUserByDiscordId(badge.authorDiscordId);
		if (!author) {
			logger.warn(`Author not found for badge with fileName: ${fileName}, skipping`);
			continue;
		}
