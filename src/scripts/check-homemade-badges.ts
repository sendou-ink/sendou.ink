...
import badgesJson from '../app/features/badges/homemade.json' with { type: 'json' };
...
if (!parsedBadges.success) {
	console.error('Invalid homemade.json format', JSON.stringify(parsedBadges.error));
	process.exit(1);
}
...
for (const fileName of Object.keys(badges)) {
	for (const ext of ['png', 'avif', 'gif']) {
		...
		if (!fs.existsSync(filePath)) {
			console.error(`Missing file for badge ${fileName}: ${filePath} does not exist`);
			process.exit(1);
		}
	}
}
