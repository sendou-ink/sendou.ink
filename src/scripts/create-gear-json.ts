...
async function main() {
	...
	const headIds = Object.fromEntries(
		brandIds.map((brand) => [brand, headGear.filter((w) => w.brand === brand).map((w) => w.id)])
	);
	const clothesIds = Object.fromEntries(
		brandIds.map((brand) => [brand, clothesGear.filter((w) => w.brand === brand).map((w) => w.id)])
	);
	const shoesIds = Object.fromEntries(
		brandIds.map((brand) => [brand, shoesGear.filter((w) => w.brand === brand).map((w) => w.id)])
	);
