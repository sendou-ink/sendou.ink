// TODO: migrate to Kysely

import type { Tables, UserWithPlusTier } from '$lib/server/db/tables';
import { weaponIdToAltId } from '$lib/constants/in-game/weapon-ids';
...
import { cache, syncCached } from '$lib/utils/cache.server';
...
function augmentBuild<T>({
	...
	...row
}: T & { modes: string; weapons: string; abilities: string; updatedAt: number }) {
	const modes = rawModes ? (JSON.parse(rawModes) as ModeShort[]) : null;
	const weapons = (JSON.parse(rawWeapons) as Array<BuildWeaponWithTop500Info>).sort(
		(a, b) => a.weaponSplId - b.weaponSplId
	);
	...
	return {
		...row,
		updatedAt: databaseTimestampToDate(row.updatedAt),
		modes,
		weapons,
