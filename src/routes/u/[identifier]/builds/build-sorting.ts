...
import { weaponIdToBaseWeaponId } from '$lib/constants/in-game/weapon-ids';
import type * as BuildRepository from '$lib/server/db/repositories/build';
import type { BuildSort } from '$lib/server/db/tables';
...
export function sortBuilds({ builds, buildSorting, weaponPool }: SortBuildsArgs) {
	const sorters: Record<
		BuildSort,
		(a: SortBuildsArgs['builds'][number], b: SortBuildsArgs['builds'][number]) => number
	> = {
		...
		UPDATED_AT: (a, b) => b.updatedAt.getTime() - a.updatedAt.getTime(),
		...
		MODE: (a, b) => {
			const aLowestModeIdx = modesShort.findIndex((mode) => a.modes?.includes(mode));
			const bLowestModeIdx = modesShort.findIndex((mode) => b.modes?.includes(mode));
