...
import { db } from '../sql';
import type { Tables, TablesInsertable, UserMapModePreferences } from '../tables';
import { COMMON_USER_FIELDS } from '$lib/utils/kysely.server';
import { shortNanoid } from '$lib/utils/id';

// xxx: actual type
type LookingGroupWithInviteCode = any;
...
export function mapModePreferencesByGroupId(groupId: number) {
	return db
		...
		.execute() as Promise<{ userId: number; preferences: UserMapModePreferences }[]>;
}

// groups visible for longer to make development easier
const SECONDS_TILL_STALE = process.env.NODE_ENV === 'development' ? 1_000_000 : 1_800;
...
export async function findLookingGroups({
	...
	const rows = await db
		...
		.where((eb) =>
			eb.or([
				eb('Group.latestActionAt', '>', sql<Date>`(unixepoch() - ${SECONDS_TILL_STALE})`),
	...
	return rows
		.map((row) => {
			return {
				...
				mapModePreferences: row.mapModePreferences?.map((c) => c.mapModePreferences) as NonNullable<
					Tables['User']['mapModePreferences']
				>[],
...
export async function createGroupFromPrevious(args: CreateGroupFromPreviousGroupArgs) {
	return db.transaction().execute(async (trx) => {
		const createdGroup = await trx
			.insertInto('Group')
...
export function upsertPrivateUserNote(args: TablesInsertable['PrivateUserNote']) {
	return db
...
export async function usersThatTrusted(userId: number) {
	...
	const rows = await db
		...
		.select([...COMMON_USER_FIELDS, 'User.inGameName', 'TeamMemberWithSecondary.teamId'])
		.where(
		...
		.union((eb) =>
			eb
				...
				.innerJoin('UserFriendCode', 'UserFriendCode.userId', 'User.id')
				.select([...COMMON_USER_FIELDS, 'User.inGameName', sql.raw<any>('null').as('teamId')])
				.where('TrustRelationship.trustReceiverUserId', '=', userId)
		)
		.execute();

	// xxx: add banning logic
	// const rowsWithoutBanned = rows.filter((row) => !userIsBanned(row.id));
	const rowsWithoutBanned = rows;

	const teamMemberIds = rowsWithoutBanned.filter((row) => row.teamId).map((row) => row.id);
...
export async function setOldGroupsAsInactive() {
	...
	return db.transaction().execute(async (trx) => {
		const groupsToSetInactive = await trx
			...
			.where('GroupMatch.id', 'is', null)
			.where('latestActionAt', '<', oneHourAgo)
			.execute();
