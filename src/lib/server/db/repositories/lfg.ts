...
import { LFG } from '$lib/constants/lfg';
...
export async function posts(user?: { id: number; plusTier: number | null }) {
	...
	const rows = await db
		...
		.where((eb) =>
			eb.or([eb('LFGPost.updatedAt', '>', postExpiryCutoff()), eb('LFGPost.authorId', '=', userId)])
		)
...
export function deletePostsByTeamId(teamId: number, trx?: Transaction<DB>) {
	return (trx ?? db).deleteFrom('LFGPost').where('teamId', '=', teamId).execute();
}
