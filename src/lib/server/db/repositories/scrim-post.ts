...
import type { AssociationVisibility, Tables, TablesInsertable } from '../tables';
...
import * as Scrim from '$lib/core/scrim/Scrim';
import { getPostRequestCensor, parseLutiDiv } from '$lib/utils/scrim';
import type { Unwrapped } from '$lib/utils/types';
import type { ScrimPost, ScrimPostUser } from '$lib/core/scrim/types';
	...
	return db.transaction().execute(async (trx) => {
		const newPost = await trx
			...
			.values({
				...
				managedByAnyone: args.managedByAnyone,
				isScheduledForFuture: args.isScheduledForFuture
			})
			.returning('id')
...
type InsertRequestArgs = Pick<Insertable<Tables['ScrimPostRequest']>, 'scrimPostId' | 'teamId'> & {
	users: Array<Pick<Insertable<TablesInsertable['ScrimPostRequestUser']>, 'userId' | 'isOwner'>>;
};
				...
				.select((innerEb) => [
					...
					jsonArrayFrom(
						innerEb
							...
							.whereRef('ScrimPostRequestUser.scrimPostRequestId', '=', 'ScrimPostRequest.id')
					).as('users')
				])
...
function mapDBRowToScrimPost(row: Unwrapped<typeof findMany> & { chatCode?: string }): ScrimPost {
	const someRequestIsAccepted = row.requests.some((request) => request.isAccepted);
	...
	const managerIds = row.managedByAnyone ? users.map((user) => user.id) : ownerIds;
	...
	return {
		...
		requests: requests.map((request) => {
			return {
				...
				createdAt: request.createdAt as unknown as Date, // xxx: not date?
...
export async function findAllRelevant(userId?: number): Promise<ScrimPost[]> {
	...
	const mapped = rows
		...
		.filter((post) => !Scrim.isAccepted(post) || (userId && Scrim.isParticipating(post, userId)));
...
export function deleteRequest(scrimPostRequestId: number) {
	return db.deleteFrom('ScrimPostRequest').where('id', '=', scrimPostRequestId).execute();
}
