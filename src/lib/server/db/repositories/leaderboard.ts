...
import { ordinalToSp } from '$lib/core/mmr/utils';
...
function teamLeaderboardBySeasonQuery(season: number) {
	return db
		.selectFrom('Skill')
		.innerJoin(
			(eb) =>
				eb
					...
					.select(({ fn }) => ['InnerSkill.identifier', fn.max('InnerSkill.id').as('maxId')])
					.where('season', '=', season)
		...
		.select((eb) => [
			...
			jsonArrayFrom(
				eb
					.selectFrom('SkillTeamUser')
					.innerJoin('User', 'SkillTeamUser.userId', 'User.id')
					.innerJoin('TeamMemberWithSecondary', 'TeamMemberWithSecondary.userId', 'User.id')
...
async function filterOutNonSqPlayers(args: {
	...
	const validUserIds = await userIdsWithEnoughSqMatchesForTeamLeaderboard(args.season);
...
async function userIdsWithEnoughSqMatchesForTeamLeaderboard(seasonNth: number) {
	...
	const userIds = await db
		...
		.where('GroupMatch.createdAt', '>', season.starts)
		.where(
			'GroupMatch.createdAt',
			'<',
			add(season.ends, { days: 1 }) // some matches can be finished after the season ends
		)
		.execute();
...
export async function seasonsParticipatedInByUserId(userId: number) {
	const rows = await db
		...
		.where('userId', '=', userId)
		.where(({ or, eb }) =>
			or([eb('groupMatchId', 'is not', null), eb('tournamentId', 'is not', null)])
		)
		.groupBy('season')
		.orderBy('season', 'desc')
		.execute();
