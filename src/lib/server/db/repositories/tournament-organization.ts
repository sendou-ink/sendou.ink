import { TOURNAMENT_SERIES_EVENTS_PER_PAGE } from '$lib/constants/organization';
import { db } from '$lib/server/db/sql';
import type { Tables, TablesInsertable } from '$lib/server/db/tables';
...
export async function findBySlug(slug: string) {
	const organization = await db
		...
		.leftJoin('UserSubmittedImage', 'UserSubmittedImage.id', 'TournamentOrganization.avatarImgId')
		.select(({ eb }) => [
			...
			jsonArrayFrom(
				eb
					...
					.whereRef('TournamentOrganizationMember.organizationId', '=', 'TournamentOrganization.id')
			).as('members'),
			jsonArrayFrom(
				eb
					...
					.whereRef('TournamentOrganizationSeries.organizationId', '=', 'TournamentOrganization.id')
			).as('series'),
			jsonArrayFrom(
				eb
					.selectFrom('TournamentOrganizationBadge')
					.innerJoin('Badge', 'Badge.id', 'TournamentOrganizationBadge.badgeId')
					.select(['Badge.id', 'Badge.displayName', 'Badge.code'])
					.whereRef('TournamentOrganizationBadge.organizationId', '=', 'TournamentOrganization.id')
			).as('badges')
		])
...
function findEventsBaseQuery(organizationId: number) {
	return db
		...
		.select(({ eb }) => [
			...
			jsonObjectFrom(
				eb
					...
					.leftJoin('UserSubmittedImage as u2', 'TournamentTeam.avatarImgId', 'u2.id')
					.select(({ eb: innerEb }) => [
						'TournamentTeam.name',
					...
					.whereRef('TournamentResult.tournamentId', '=', 'CalendarEvent.tournamentId')
					...
					.select(({ eb: innerEb }) => [
						...
						jsonArrayFrom(
							innerEb
								...
								.innerJoin('User', 'User.id', 'CalendarEventResultPlayer.userId')
								.select(['User.discordAvatar', 'User.discordId'])
...
>(event: T) {
	return {
		...event,
		logoUrl: !event.tournamentId ? null : event.logoUrl ? userSubmittedImage(event.logoUrl) : '' // xxx: fix HACKY_
	};
}

export async function findEventsByMonth({ month, year, organizationId }: FindEventsByMonthArgs) {
	...
	const events = await findEventsBaseQuery(organizationId)
		.where('CalendarEventDate.startTime', '>=', firstDayOfTheMonth)
		.where('CalendarEventDate.startTime', '<=', lastDayOfTheMonth)
...
function findSeriesEventsBaseQuery({
	organizationId,
	substringMatches
}: {
	organizationId: number;
	...
	return findEventsBaseQuery(organizationId)
		.where((eb) =>
			eb.or(substringMatches.map((match) => eb('CalendarEvent.name', 'like', `%${match}%`)))
		)
...
interface UpdateArgs
	extends Pick<Tables['TournamentOrganization'], 'id' | 'name' | 'description' | 'socials'> {
	members: Array<
		Pick<Tables['TournamentOrganizationMember'], 'role' | 'roleDisplayName' | 'userId'>
	>;
	series: Array<
...
export function update({ id, name, description, socials, members, series, badges }: UpdateArgs) {
	return db.transaction().execute(async (trx) => {
		...
		await trx.deleteFrom('TournamentOrganizationMember').where('organizationId', '=', id).execute();
		...
		await trx.deleteFrom('TournamentOrganizationSeries').where('organizationId', '=', id).execute();
 ...
 */
export function unbanUser({ organizationId, userId }: { organizationId: number; userId: number }) {
	return db
...
export function allBannedUsersByOrganizationId(organizationId: number) {
	return db
		...
		.where('TournamentOrganizationBannedUser.organizationId', '=', organizationId)
