...
import { nullFilledArray, nullifyingAvg } from '$lib/utils/arrays';
import { shortNanoid } from '$lib/utils/id';
import { COMMON_USER_FIELDS, userChatNameColor } from '$lib/utils/kysely.server';
import type { Unwrapped } from '$lib/utils/types';
import { userSubmittedImage } from '$lib/utils/urls-img';
import { sub } from 'date-fns';
import { sql, Transaction, type NotNull } from 'kysely';
	...
	const result = await db
		...
		.select(({ eb }) => [
			...
			eb
				...
				.whereRef('CalendarEvent.avatarImgId', '=', 'UnvalidatedUserSubmittedImage.id')
				.as('logoUrl'),
			eb
				.selectFrom('UnvalidatedUserSubmittedImage')
			...
			jsonArrayFrom(
				eb
					...
					.select([...COMMON_USER_FIELDS, userChatNameColor, 'TournamentStaff.role'])
					.where('TournamentStaff.tournamentId', '=', id)
			).as('staff'),
			jsonArrayFrom(
				eb
					.selectFrom('TournamentSub')
					.select(({ fn }) => ['TournamentSub.visibility', fn.countAll<number>().as('count')])
			...
			jsonArrayFrom(
				eb
					...
					.whereRef('TournamentBracketProgressionOverride.tournamentId', '=', 'Tournament.id')
			).as('bracketProgressionOverrides'),
			jsonArrayFrom(
				eb
					.selectFrom('TournamentTeam')
					.leftJoin('UserSubmittedImage', 'TournamentTeam.avatarImgId', 'UserSubmittedImage.id')
					.select(({ eb: innerEb }) => [
						'TournamentTeam.id',
						'TournamentTeam.name',
						'TournamentTeam.seed',
							...
							innerEb
								...
								.leftJoin('SeedingSkill', (join) =>
									join
										...
										.on('SeedingSkill.type', '=', isSetAsRanked ? 'RANKED' : 'UNRANKED')
								)
								.select([
									'User.id as userId',
									'User.username',
                    ...
                    "User"."inGameName"
						...
						jsonArrayFrom(
							innerEb
								...
								.whereRef('TournamentTeamCheckIn.tournamentTeamId', '=', 'TournamentTeam.id')
						).as('checkIns'),
						jsonArrayFrom(
							innerEb
						...
						jsonObjectFrom(
							innerEb
								.selectFrom('AllTeam')
								.leftJoin('UserSubmittedImage', 'AllTeam.avatarImgId', 'UserSubmittedImage.id')
			...
			jsonArrayFrom(
				eb
					...
					.innerJoin('TournamentMatch', 'TournamentMatch.stageId', 'TournamentStage.id')
					.innerJoin(
		...
		.select((eb) => [
			...
			jsonArrayFrom(
				eb
					...
					.whereRef('TournamentOrganization.id', '=', 'CalendarEvent.organizationId')
					.where('TournamentOrganizationMember.role', 'in', ['ADMIN', 'ORGANIZER'])
...
export async function findCastTwitchAccountsByTournamentId(tournamentId: number) {
	const result = await db
		.selectFrom('Tournament')
...
export function checkedInTournamentTeamsByBracket({
	...
	return db
		.selectFrom('TournamentTeamCheckIn')
		.innerJoin('TournamentTeam', 'TournamentTeamCheckIn.tournamentTeamId', 'TournamentTeam.id')
...
export async function friendCodesByTournamentId(tournamentId: number) {
	const values = await db
		.selectFrom('TournamentTeam')
		.innerJoin('TournamentTeamMember', 'TournamentTeam.id', 'TournamentTeamMember.tournamentTeamId')
		.innerJoin('UserFriendCode', 'TournamentTeamMember.userId', 'UserFriendCode.userId')
	...
	return db.transaction().execute(async (trx) => {
		...
		if (
			...
			await trx
				...
				.where('TournamentTeamCheckIn.tournamentTeamId', 'in', allTournamentTeamsOfTournament)
				.execute();
...
export function upsertPreparedMaps({ bracketIdx, maps, tournamentId }: UpsertPreparedMapsArgs) {
	return db.transaction().execute(async (trx) => {
		...
		const preparedMaps: Array<PreparedMaps | null> =
			tournament.preparedMaps ?? nullFilledArray(tournament.settings.bracketProgression.length);

		preparedMaps[bracketIdx] = { ...maps, createdAt: new Date() };
...
async function castedMatchesInfoByTournamentId(trx: Transaction<DB>, tournamentId: number) {
	return (
		(
			await trx
				.selectFrom('Tournament')
	...
	return db.transaction().execute(async (trx) => {
		...
		if (twitchAccount === null) {
			newCastedMatchesInfo = {
				...castedMatchesInfo,
				castedMatches: castedMatchesInfo.castedMatches.filter((cm) => cm.matchId !== matchId)
			};
		} else {
			newCastedMatchesInfo = {
				...castedMatchesInfo,
...
export function addPickBanEvent(values: TablesInsertable['TournamentMatchPickBanEvent']) {
...
export function resetBracket(tournamentStageId: number) {
	return db.transaction().execute(async (trx) => {
		await trx.deleteFrom('TournamentMatch').where('stageId', '=', tournamentStageId).execute();

		await trx.deleteFrom('TournamentRound').where('stageId', '=', tournamentStageId).execute();

		await trx.deleteFrom('TournamentGroup').where('stageId', '=', tournamentStageId).execute();

		await trx.deleteFrom('TournamentStage').where('id', '=', tournamentStageId).execute();
	});
}

export type TournamentRepositoryInsertableMatch = Omit<
	TablesInsertable['TournamentMatch'],
	'status' | 'chatCode'
>;

export function insertSwissMatches(matches: TournamentRepositoryInsertableMatch[]) {
	if (matches.length === 0) {
...
export function deleteSwissMatches({ groupId, roundId }: { groupId: number; roundId: number }) {
	return db
