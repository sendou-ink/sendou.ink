...
import { db } from '../sql';
import type { AssociationVirtualIdentifier } from '$lib/constants/association';
import { logger } from '$lib/utils/logger';
import type { TablesInsertable, TablesUpdatable } from '../tables';
...
export async function findByInviteCode(
	...
	const associations = await findBy({ type: 'inviteCode', inviteCode }, options);

	return associations.at(0);
}

function baseFindQuery(options: FindOptions) {
	return db
...
async function findBy(
	...
	const associations =
		args.type === 'user'
			? await baseFindQuery(options).where('AssociationMember.userId', '=', args.userId).execute()
			: args.type === 'inviteCode'
				? await baseFindQuery(options)
						.where('Association.inviteCode', '=', args.inviteCode)
						.execute()
				: await baseFindQuery(options).where('Association.id', '=', args.associationId).execute();

	return associations.map((a) => ({
		...a,
		permissions: {
			MANAGE: (a.members ?? []).filter((member) => member.role === 'ADMIN').map((user) => user.id)
		}
	}));
}
...
export function update(associationId: number, args: Partial<TablesUpdatable['Association']>) {
	return db.updateTable('Association').set(args).where('id', '=', associationId).execute();
}
...
export function addMember({ associationId, userId }: { associationId: number; userId: number }) {
	return db
		.insertInto('AssociationMember')
...
export function removeMember({ associationId, userId }: { associationId: number; userId: number }) {
	return db
