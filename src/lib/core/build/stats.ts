...
export function abilityPointCountsToAverages({
	...
	invariant(allBuildsCount > weaponBuildsCount, 'allBuildsCount < weaponBuildsCount');

	const mainOnlyAbilities = abilities
		.filter((ability) => ability.type !== 'STACKABLE')
		.map((ability) => {
			const allBuildsAbilityPoints = toAbilityPoints(allAbilities, ability.name);
			const weaponBuildsAbilityPoints = toAbilityPoints(weaponAbilities, ability.name);
			...
			return {
				name: ability.name,
				percentage: {
					all: roundToNDecimalPlaces((allBuildsWithTheAbility / allBuildsCount) * 100),
					weapon: roundToNDecimalPlaces((weaponBuildsWithTheAbility / weaponBuildsCount) * 100)
				}
			};
		})
		...
		.map((ability) => {
			...
			return {
				...
				apAverage: {
					...
					weapon: roundToNDecimalPlaces(weaponBuildsAbilityPoints / weaponBuildsCount)
				}
			};
		})
	...
	return serializedToShow.map(([serialized, count]) => {
		const abilities = serialized.split(',').map((serializedAbility) => {
			...
			return {
				...
				count: isStackableAbility(ability as Ability) ? Number(count) : undefined
			};
		});
