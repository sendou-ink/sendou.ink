...
import type * as TeamRepository from '$lib/server/db/repositories/team';
...
export function canAddCustomizedColors(team: { members: { patronTier: number | null }[] }) {
	return team.members.some((member) => member.patronTier && member.patronTier >= 2);
}
...
export function subsOfResult<T extends { id: number }>(
	result: { participants: Array<T>; startTime: Date },
