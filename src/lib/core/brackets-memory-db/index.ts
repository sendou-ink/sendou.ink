import type { CrudInterface, Database, OmitId, Table } from '$lib/core/brackets-manager';
...
export class InMemoryDatabase implements CrudInterface {
	...
	makeFilter(partial: any): (entry: any) => boolean {
		return (entry: any): boolean => {
			let result = true;
			for (const key of Object.keys(partial)) result = result && entry[key] === partial[key];
	...
	select<T>(table: Table, arg?: number | Partial<T>): T[] | null {
		try {
			...
			// @ts-expect-error imported
			return this.data[table].filter(this.makeFilter(arg)).map((val) => structuredClone(val));
		} catch {
			return null;
		}
	}
	 ...
	 * @param value How to update.
	 */
	update<T>(table: Table, arg: number | Partial<T>, value?: Partial<T>): boolean {
		if (typeof arg === 'number') {
			if (
				this.data[table][arg] &&
				value &&
	...
	delete<T>(table: Table, filter?: Partial<T>): boolean {
		...
		function negativeFilter(value: any): boolean {
