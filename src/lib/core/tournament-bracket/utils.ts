...
export function everyMatchIsOver(bracket: Pick<TournamentManagerDataSet, 'match'>) {
	// winners, losers & grand finals+bracket reset are all different stages
	const isDoubleElimination = R.unique(bracket.match.map((match) => match.group_id)).length === 3;
...
export function everyBracketOver(tournament: TournamentManagerDataSet) {
	...
	for (const stageId of stageIds) {
		const matches = tournament.match.filter((match) => match.stage_id === stageId);
...
export function bracketHasStarted(bracket: TournamentManagerDataSet) {
	return bracket.stage[0] && bracket.stage[0].id !== 0;
}
...
// ) {
// 	if (team.activeRosterUserIds && team.activeRosterUserIds.length === teamMinMemberCount) {
...
// }) {
// 	if (badgeReceivers.some((receiver) => !badges.some((badge) => badge.id === receiver.badgeId))) {
// 		return 'BADGE_NOT_FOUND';
// 	}

// 	for (const badge of badges) {
// 		const owner = badgeReceivers.find((receiver) => receiver.badgeId === badge.id);
// 		if (!owner || owner.userIds.length === 0) {
// 			return 'BADGE_NOT_ASSIGNED';
// 		}
// 	}

// 	const tournamentTeamIds = badgeReceivers.map((receiver) => receiver.tournamentTeamId);
