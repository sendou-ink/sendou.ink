...
export function validatedBracketsToInputFormat(brackets: ParsedBracket[]): InputBracket[] {
...
export function validatedBrackets(brackets: InputBracket[]): ParsedBracket[] | ValidationError {
...
export function bracketsToValidationError(brackets: ParsedBracket[]): ValidationError | null {
...
function toOutputBracketFormat(brackets: InputBracket[]): ParsedBracket[] {
	...
	invariant(
		result.every(
			(bracket) => !bracket.sources || bracket.sources.every((source) => source.bracketIdx >= 0),
...
function tooManyPlacements(brackets: ParsedBracket[]) {
	...
	const size = Math.min(
		...roundRobins.map(
			(bracketIdx) => brackets[bracketIdx].settings.teamsPerGroup ?? Number.POSITIVE_INFINITY
		)
	);
...
export function isBrackets(input: ParsedBracket[] | ValidationError): input is ParsedBracket[] {
...
export function isError(input: ParsedBracket[] | ValidationError): input is ValidationError {
	return !Array.isArray(input);
}
	...
	while (true) {
		const bracket = brackets.findIndex((bracket) =>
			bracket.sources?.some(
				(source) => source.placements.includes(1) && source.bracketIdx === bracketIdxToFind
			)
		);
...
export function bracketIdxsForStandings(progression: ParsedBracket[]) {
	...
	const withoutIntermediateBrackets = bracketsToConsider.filter((bracket, bracketIdx) => {
		if (bracketIdx === 0) return true;

		return progression.every((b) => !b.sources?.some((s) => s.bracketIdx === bracket));
	});

	const withoutUnderground = withoutIntermediateBrackets.filter((bracketIdx) => {
		const sources = progression[bracketIdx].sources;

		if (!sources) return true;

		return !sources.some((source) => progression[source.bracketIdx].type === 'double_elimination');
	});

	return withoutUnderground.sort((a, b) => {
		const minSourcedPlacementA = Math.min(
			...(progression[a].sources?.flatMap((s) => s.placements) ?? [Number.POSITIVE_INFINITY])
		);
		const minSourcedPlacementB = Math.min(
			...(progression[b].sources?.flatMap((s) => s.placements) ?? [Number.POSITIVE_INFINITY])
		);
...
function bracketsReachableFrom(bracketIdx: number, progression: ParsedBracket[]): number[] {
...
export function destinationByPlacement({
	...
	const destinations = destinationsFromBracketIdx(sourceBracketIdx, progression);
