...
describe('bracketsToValidationError - valid formats', () => {
	it('accepts SE', () => {
		expect(Progression.bracketsToValidationError(progressions.singleElimination)).toBeNull();
	});

	it('accepts RR->SE', () => {
		expect(
			Progression.bracketsToValidationError(progressions.roundRobinToSingleElimination)
		).toBeNull();
	});

	it('accepts low ink', () => {
		expect(Progression.bracketsToValidationError(progressions.lowInk)).toBeNull();
	});

	it('accepts many starter brackets', () => {
		expect(Progression.bracketsToValidationError(progressions.manyStartBrackets)).toBeNull();
	});

	it('accepts swiss (one group)', () => {
		expect(Progression.bracketsToValidationError(progressions.swissOneGroup)).toBeNull();
	});
});

describe('validatedSources - PLACEMENTS_PARSE_ERROR', () => {
	function getValidatedBracketsFromPlacements(placements: string) {
	...
	it('parses placements correctly (separated by comma)', () => {
		const result = getValidatedBracketsFromPlacements('1,2,3,4') as Progression.ParsedBracket[];

		expect(result[1].sources).toEqual([{ bracketIdx: 0, placements: [1, 2, 3, 4] }]);
	});

	it('parses placements correctly (separated by line)', () => {
		const result = getValidatedBracketsFromPlacements('1-4') as Progression.ParsedBracket[];
	...
	it('handles placement where ranges start and end is the same', () => {
		const result = getValidatedBracketsFromPlacements('1-1') as Progression.ParsedBracket[];
	...
	it('handles parsing with negative placements', () => {
		...
		expect(result[1].sources).toEqual([{ bracketIdx: 0, placements: [-1, -2] }]);
	});
...
describe('isFinals', () => {
	...
	it('handles RR->SE', () => {
		expect(Progression.isFinals(0, progressions.roundRobinToSingleElimination)).toBe(false);
		expect(Progression.isFinals(1, progressions.roundRobinToSingleElimination)).toBe(true);
	});
	...
	it('throws if given idx is out of bounds', () => {
		expect(() => Progression.isFinals(1, progressions.singleElimination)).toThrow();
	});
});

describe('isUnderground', () => {
	it('handles SE', () => {
		expect(Progression.isUnderground(0, progressions.singleElimination)).toBe(false);
	});

	it('handles RR->SE', () => {
		expect(Progression.isUnderground(0, progressions.roundRobinToSingleElimination)).toBe(false);
		expect(Progression.isUnderground(1, progressions.roundRobinToSingleElimination)).toBe(false);
	});
	...
	it('many starter brackets', () => {
		expect(Progression.isUnderground(0, progressions.manyStartBrackets)).toBe(false);
		expect(Progression.isUnderground(1, progressions.manyStartBrackets)).toBe(true);
		expect(Progression.isUnderground(2, progressions.manyStartBrackets)).toBe(false);
		expect(Progression.isUnderground(3, progressions.manyStartBrackets)).toBe(true);
	});

	it('throws if given idx is out of bounds', () => {
		expect(() => Progression.isUnderground(1, progressions.singleElimination)).toThrow();
	});
});
...
describe('changedBracketProgression', () => {
	it('reports changed bracket indexes', () => {
		...
		expect(Progression.changedBracketProgression(progressions.lowInk, withChanges)).toEqual([0, 1]);
	});

	it('returns an empty array if nothing changed', () => {
		expect(Progression.changedBracketProgression(progressions.lowInk, progressions.lowInk)).toEqual(
			[]
		);
	});
});
...
describe('bracketIdxsForStandings', () => {
	it('handles SE', () => {
		expect(Progression.bracketIdxsForStandings(progressions.singleElimination)).toEqual([0]);
	});

	it('handles RR->SE', () => {
		expect(Progression.bracketIdxsForStandings(progressions.roundRobinToSingleElimination)).toEqual(
			[1, 0]
		);
	});
	...
	it('handles many starter brackets', () => {
		expect(Progression.bracketIdxsForStandings(progressions.manyStartBrackets)).toEqual([2, 0]); // NOTE, 3,1 excluded because they are not in the main progression
	});

	it('handles swiss (one group)', () => {
		expect(Progression.bracketIdxsForStandings(progressions.swissOneGroup)).toEqual([0]);
	});

	it('handles DE w/ underground bracket', () => {
		expect(
			Progression.bracketIdxsForStandings(progressions.doubleEliminationWithUnderground)
...
describe('destinationsFromBracketIdx', () => {
	it('returns correct destination (one destination)', () => {
		expect(
			Progression.destinationsFromBracketIdx(0, progressions.roundRobinToSingleElimination)
		).toEqual([1]);
	});

	it('returns correct destination (many destinations)', () => {
		expect(Progression.destinationsFromBracketIdx(0, progressions.lowInk)).toEqual([1, 2]);
	});

	it('returns an empty array if no destinations', () => {
		expect(Progression.destinationsFromBracketIdx(0, progressions.singleElimination)).toEqual([]);
	});
});
