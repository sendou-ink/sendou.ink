...
import type { Tables, TournamentRoundMaps } from '$lib/server/db/tables';
import { logger } from '$lib/utils/logger';
import { assertUnreachable } from '$lib/utils/types';
import * as R from 'remeda';

// xxx: a bit confusing with tournament-map-list, can we unify these and sendouq map list and map list generation in /maps?
...
export type TournamentRoundMapList = ReturnType<typeof generateTournamentRoundMaplist>;

// TODO: future improvement could be slightly biasing against maps that appear in slots that are not guaranteed to be played
export function generateTournamentRoundMaplist(args: GenerateTournamentRoundMaplistArgs) {
	...
	for (const [iteration, round] of sortedRounds.entries()) {
		...
		function amountOfMapsToGenerate() {
		...
		result.set(round.id, {
			count,
			pickBan: args.roundsWithPickBan.has(round.id) ? args.pickBanStyle : undefined,
			list:
				// teams pick
				args.pool.length === 0
					? null
					: // TO pick
						modes.map((mode) => ({
							mode,
							stageId: resolveStage(mode, args.pool, stageAppearance, comboAppearance, iteration)
						}))
		});
	}
...
function getFilteredRounds(rounds: Round[], type: Tables['TournamentStage']['type']) {
...
function resolveRoundMapCount(
	...
	if (typeof count === 'undefined') {
		logger.warn(`No map count found for round ${round.number} (group ${round.group_id})`);
		return 5;
	}
...
function serializedMap(mode: ModeShort, stage: StageId) {
	return `${mode}-${stage}`;
}
