...
describe('round robin standings', () => {
	it('should sort teams primarily by set wins (per group) in paddling pool 255', () => {
		...
		expect(groupIds.length, 'Paddling Pool 255 should have groups from Group A to Group I').toBe(9);

		for (const groupId of groupIds) {
			const groupStandings = standings.filter((standing) => standing.groupId === groupId);
	...
	it('has ascending order from lower group id to higher group id for same placements', () => {
		...
		const placements = R.unique(standings.map((standing) => standing.placement)).sort(
			(a, b) => a - b
		);

		for (const placement of placements) {
			const placementStandings = standings.filter((standing) => standing.placement === placement);
