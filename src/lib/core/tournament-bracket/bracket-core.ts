...
import { getTournamentManager } from '$lib/core/tournament/utils';
import type { Round } from '$lib/core/brackets-model';
import { fillWithNullTillPowerOfTwo } from '$lib/core/tournament-bracket/utils';
...
export abstract class BracketCore {
		...
		try {
			...
			while (matchesToResolve) {
				...
				for (const match of manager.export().match) {
					...
					const winner =
						(teamOrder.get(match.opponent1.id!) ?? 0) < (teamOrder.get(match.opponent2.id!) ?? 0)
							? 1
							: 2;
	...
	simulatedMatch(matchId: number) {
		...
		return this.simulatedData.match.filter(Boolean).find((match) => match.id === matchId);
	}
	...
	get participantTournamentTeamIds() {
		return R.unique(
			this.data.match.flatMap((match) => [match.opponent1?.id, match.opponent2?.id]).filter(Boolean)
		) as number[];
	}
	...
	generateMatchesData(teams: number[]) {
		...
		if (teams.length >= TOURNAMENT.ENOUGH_TEAMS_TO_START) {
			manager.create({
				...
				name: 'Virtual',
				type: this.type,
				seeding: this.type === 'round_robin' ? teams : fillWithNullTillPowerOfTwo(teams),
				settings: this.tournament.bracketManagerSettings(this.settings, this.type, teams.length)
			});
		}
	...
	get isUnderground() {
		return Progression.isUnderground(this.idx, this.tournament.ctx.settings.bracketProgression);
	}

	get isFinals() {
		return Progression.isFinals(this.idx, this.tournament.ctx.settings.bracketProgression);
	}
	...
	get enoughTeams() {
		return this.participantTournamentTeamIds.length >= TOURNAMENT.ENOUGH_TEAMS_TO_START;
	}
...
class SingleEliminationBracket extends BracketCore {
	...
	defaultRoundBestOfs(data: TournamentManagerDataSet) {
		...
		for (const group of data.group) {
			const roundsOfGroup = data.round.filter((round) => round.group_id === group.id);

			function defaultOfRound(round: Round) {
				// 3rd place match
				...
				// small brackets
				if (round.number === maxRoundNumber || round.number === maxRoundNumber - 1) {
					return 5;
				}
				return 3;
			}
			...
			for (const round of roundsOfGroup) {
				const atLeastOneNonByeMatch = data.match.some(
					(match) => match.round_id === round.id && match.opponent1 && match.opponent2
				);
				...
				result.get(group.id)!.set(round.number, { count: defaultOfRound(round), type: 'BEST_OF' });
			}
		}
	...
	get standings(): Standing[] {
		...
		const matches = (() => {
			...
			return this.data.match.filter((m) => m.group_id !== thirdPlaceMatch?.group_id);
		})();
		...
		for (const match of matches.sort((a, b) => a.round_id - b.round_id)) {
			...
			const loser = match.opponent1?.result === 'win' ? match.opponent2 : match.opponent1;
			invariant(loser?.id, 'Loser id not found');

			teams.push({ id: loser.id, lostAt: match.round_id });
		}

		const teamCountWhoDidntLoseYet = this.participantTournamentTeamIds.length - teams.length;
		...
		const resultWithThirdPlaceTiebroken = result
			.map((standing) => {
				if (standing.placement === 3 && thirdPlaceMatchWinner?.id !== standing.team.id) {
					return {
						...standing,
						placement: 4
					};
...
class DoubleEliminationBracket extends BracketCore {
	...
	defaultRoundBestOfs(data: TournamentManagerDataSet) {
		...
		for (const group of data.group) {
			...
			function defaultOfRound(round: Round) {
				...
				if (group.number === 2) {
					const lastRoundNumber = Math.max(...roundsOfGroup.map((round) => round.number));
	...
	get standings(): Standing[] {
		...
		for (const match of this.data.match.slice().sort((a, b) => a.round_id - b.round_id)) {
		...
		for (const roundId of R.unique(teams.map((team) => team.lostAt))) {
			...
			for (const { id: teamId } of teamsLostThisRound) {
				...
				const teamsPlacedAbove = teamCountWhoDidntLoseInLosersYet + teams.length;
		...
		const grandFinalsGroupId = this.data.group.find((g) => g.number === grandFinalsNumber)?.id;
		...
		if (
			...
			const loser = grandFinalMatches[0].opponent1.result === 'win' ? 'opponent2' : 'opponent1';
			const winner = loser === 'opponent1' ? 'opponent2' : 'opponent1';

			const loserTeam = this.tournament.teamById(grandFinalMatches[0][loser]!.id!);
			invariant(loserTeam, 'Loser team not found');
			const winnerTeam = this.tournament.teamById(grandFinalMatches[0][winner]!.id!);
			...
			const loser = grandFinalMatches[1].opponent1?.result === 'win' ? 'opponent2' : 'opponent1';
			const winner = loser === 'opponent1' ? 'opponent2' : 'opponent1';

			const loserTeam = this.tournament.teamById(grandFinalMatches[1][loser]!.id!);
			invariant(loserTeam, 'Loser team not found');
			const winnerTeam = this.tournament.teamById(grandFinalMatches[1][winner]!.id!);
			invariant(winnerTeam, 'Winner team not found');
	...
	source(placements: number[]) {
		function resolveLosersGroupId(data: TournamentManagerDataSet) {
			const minGroupId = Math.min(...data.round.map((round) => round.group_id));

			return minGroupId + 1;
		}
		function placementsToRoundsIds(data: TournamentManagerDataSet, losersGroupId: number) {
			function firstRoundIsOnlyByes() {
				const losersMatches = data.match.filter((match) => match.group_id === losersGroupId);

				const fistRoundId = Math.min(...losersMatches.map((m) => m.round_id));

				const firstRoundMatches = losersMatches.filter((match) => match.round_id === fistRoundId);
			...
			const losersRounds = data.round.filter((round) => round.group_id === losersGroupId);
			const orderedRoundsIds = losersRounds.map((round) => round.id).sort((a, b) => a - b);
			const amountOfRounds = Math.abs(Math.min(...placements)) + (firstRoundIsOnlyByes() ? 1 : 0);
		...
		const sourceRoundsIds = placementsToRoundsIds(this.data, losersGroupId).sort(
		...
		for (const roundId of sourceRoundsIds) {
			const roundsMatches = this.data.match.filter((match) => match.round_id === roundId);
...
class RoundRobinBracket extends BracketCore {
	...
	source(placements: number[]): {
		...
		const relevantMatchesFinished = standings.length === this.participantTournamentTeamIds.length;

		const uniquePlacements = R.unique(standings.map((s) => s.placement));

		// 1,3,5 -> 1,2,3 e.g.
		function placementNormalized(p: number) {
	...
	currentStandings(includeUnfinishedGroups = false) {
		...
		for (const groupId of groupIds) {
			const matches = this.data.match.filter((match) => match.group_id === groupId);
			...
			for (const match of matches) {
				...
				const winner = match.opponent1?.result === 'win' ? match.opponent1 : match.opponent2;
				...
				if (typeof winner.totalPoints !== 'number' || typeof loser.totalPoints !== 'number') {
					logger.warn('RoundRobinBracket.standings: winner or loser points not found');
				}
	...
	defaultRoundBestOfs(data: TournamentManagerDataSet) {
		...
		for (const round of data.round) {
			...
			result.get(round.group_id)!.set(round.number, { count: 3, type: 'BEST_OF' });
		}
...
class SwissBracket extends BracketCore {
	...
	source(placements: number[]): {
		...
		const relevantMatchesFinished = this.data.round.every((round) => {
			const roundsMatches = this.data.match.filter((match) => match.round_id === round.id);
		...
		for (const groupId of groupIds) {
			...
			for (const match of matches) {
				if (match.opponent1?.id && match.opponent2?.id) {
					...
					matchUps.set(match.opponent1.id, [...opponentOneMatchUps, match.opponent2.id]);
					matchUps.set(match.opponent2.id, [...opponentTwoMatchUps, match.opponent1.id]);
				}
			...
			for (const match of matches) {
				...
				const round = this.data.round.find((round) => round.id === match.round_id);
				const mapWins =
				...
				for (const team2 of teams) {
					...
					const finishedMatchBetweenTeams = matches.find((match) => {
						const isBetweenTeams =
							(match.opponent1?.id === team.id && match.opponent2?.id === team2.id) ||
							(match.opponent1?.id === team2.id && match.opponent2?.id === team.id);
			...
			placements.push(
				...teams
					.sort((a, b) => {
						...
						const aOpponentSetWinPercentage = this.trackRecordToWinPercentage(a.opponentSets);
						const bOpponentSetWinPercentage = this.trackRecordToWinPercentage(b.opponentSets);
						...
						const aOpponentMapWinPercentage = this.trackRecordToWinPercentage(a.opponentMaps);
						const bOpponentMapWinPercentage = this.trackRecordToWinPercentage(b.opponentMaps);
					...
					.map((team, i) => {
						return {
							...
							stats: {
								...
								opponentSetWinPercentage: this.trackRecordToWinPercentage(team.opponentSets),
								opponentMapWinPercentage: this.trackRecordToWinPercentage(team.opponentMaps),
								points: 0
							}
						};
					})
