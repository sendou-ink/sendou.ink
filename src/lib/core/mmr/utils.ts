import { TIERS, type TierName } from '$lib/constants/mmr';
import invariant from '$lib/utils/invariant';
import { rate as openskillRate, ordinal, rating, type Rating, type Team } from 'openskill';
...
function rateConservative(
	...
	const [[ordinaryRatingForWinner], [ordinaryRatingForLoser]] = openskillRate(teams, {
		tau: TAU,
		preventSigmaIncrease: true
	});

	const [, [conservativeRatingForLoser]] = openskillRate([secondaryTeams[0], teams[1]], {
		tau: TAU,
		preventSigmaIncrease: true
	});

	const [[conservativeRatingForWinner]] = openskillRate([teams[0], secondaryTeams[1]], {
		tau: TAU,
		preventSigmaIncrease: true
	});
...
export function compareTwoTiers(tier1: TierName, tier2: TierName) {
	return (
		TIERS.findIndex(({ name }) => name === tier1) - TIERS.findIndex(({ name }) => name === tier2)
	);
}
