import type { InputStage, Stage } from '$lib/core/brackets-model';
import { create } from './create';
import { Get } from './get';
import * as helpers from './helpers';
import { Reset } from './reset';
import type { CrudInterface, Database, DataTypes, Storage, Table } from './types';
...
export class BracketsManager {
	...
	public import(rawData: Database, normalizeIds = false): void {
		...
		if (!this.storage.delete('stage')) throw Error('Could not empty the stage table.');
		if (!this.storage.insert('stage', data.stage)) throw Error('Could not import stages.');

		if (!this.storage.delete('group')) throw Error('Could not empty the group table.');
		if (!this.storage.insert('group', data.group)) throw Error('Could not import groups.');

		if (!this.storage.delete('round')) throw Error('Could not empty the round table.');
		if (!this.storage.insert('round', data.round)) throw Error('Could not import rounds.');

		if (!this.storage.delete('match')) throw Error('Could not empty the match table.');
		if (!this.storage.insert('match', data.match)) throw Error('Could not import matches.');
	}
