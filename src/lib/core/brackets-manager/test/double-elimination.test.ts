...
describe('Previous and next match update in double elimination stage', () => {
	...
	test('should determine matches in grand final', () => {
		...
		expect(storage.select<any>('match', 5).status).toBe(TournamentMatchStatus.Completed); // Grand final (round 1)
		expect(storage.select<any>('match', 6).status).toBe(TournamentMatchStatus.Ready); // Grand final (round 2)
	...
	test('should determine next matches and reset them', () => {
		...
		expect(beforeReset.opponent1.id).toBe(storage.select<any>('match', 0).opponent2.id);
	...
	test('should choose the correct previous and next matches based on losers ordering', () => {
		...
		expect(storage.select<any>('match', 8).status).toBe(TournamentMatchStatus.Completed); // WB 2.1
	});
...
describe('Skip first round', () => {
	...
	test('should choose the correct previous and next matches', () => {
		...
		for (let i = 2; i < 21; i++) manager.update.match({ id: i, opponent1: { result: 'win' } });
