...
import { TournamentMatchStatus } from '$lib/server/db/tables';
...
describe('Update matches', () => {
	...
	test('should update the status of the next match', () => {
		...
		expect(storage.select<any>('match', 8).status).toBe(TournamentMatchStatus.Waiting);
		...
		expect(storage.select<any>('match', 8).status).toBe(TournamentMatchStatus.Ready);
	});
...
describe('Locked matches', () => {
	...
	test('should throw when the matches leading to the match have not been completed yet', () => {
		...
		expect(() => manager.update.match({ id: 8 })).toThrow('The match is locked.'); // First match of WB Round 2.
		expect(() => manager.update.match({ id: 15 })).toThrow('The match is locked.'); // First match of LB Round 1.
		expect(() => manager.update.match({ id: 19 })).toThrow('The match is locked.'); // First match of LB Round 1.
		expect(() => manager.update.match({ id: 23 })).toThrow('The match is locked.'); // First match of LB Round 3.
	});
});
