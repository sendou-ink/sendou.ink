...
describe('Previous and next match update', () => {
	...
	test('should determine matches in consolation final', () => {
		...
		expect(storage.select<any>('match', 2).status).toBe(TournamentMatchStatus.Ready);
		expect(storage.select<any>('match', 3).status).toBe(TournamentMatchStatus.Ready);
	});
	...
	test('should play both the final and consolation final in parallel', () => {
		...
		expect(storage.select<any>('match', 2).status).toBe(TournamentMatchStatus.Running);
		expect(storage.select<any>('match', 3).status).toBe(TournamentMatchStatus.Ready);
		...
		expect(storage.select<any>('match', 2).status).toBe(TournamentMatchStatus.Running);
		expect(storage.select<any>('match', 3).status).toBe(TournamentMatchStatus.Running);
