...
export class Reset extends BaseUpdater {
	...
	public matchResults(matchId: number): void {
		...
		const { roundNumber, roundCount } = this.getRoundPositionalInfo(stored.round_id);
		const matchLocation = helpers.getMatchLocation(stage.type, group.number);
		const nextMatches =
			stage.type !== 'round_robin' && stage.type !== 'swiss'
				? this.getNextMatches(stored, matchLocation, stage, roundNumber, roundCount)
				: [];

		if (
			nextMatches.some(
				(match) => match && match.status >= Status.Running && !helpers.isMatchByeCompleted(match)
			)
		)
