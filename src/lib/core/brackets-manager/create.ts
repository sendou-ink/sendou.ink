...
import type { Duel, OmitId, ParticipantSlot, StandardBracketResults, Storage } from './types';
...
export class Create {
	...
	private roundRobin(): Stage {
		...
		for (let i = 0; i < groups.length; i++) this.createRoundRobinGroup(stage.id, i + 1, groups[i]);
	 ...
	 */
	private createDoubleEliminationSkipFirstRound(stageId: number, slots: ParticipantSlot[]): void {
		...
		if (helpers.isDoubleEliminationNecessary(this.stage.settings?.size!)) {
			const winnerLb = this.createLowerBracket(stageId, 2, [directInLb, ...losersWb]);
	 ...
	 * @param slots A list of slots.
	 */
	private createDoubleElimination(stageId: number, slots: ParticipantSlot[]): void {
		const { losers: losersWb, winner: winnerWb } = this.createStandardBracket(stageId, 1, slots);
	 ...
	 * @param slots A list of slots.
	 */
	private createRoundRobinGroup(stageId: number, number: number, slots: ParticipantSlot[]): void {
	 ...
	 */
	private createUniqueMatchBracket(stageId: number, number: number, duels: Duel[]): void {
		const groupId = this.insertGroup({
			stage_id: stageId,
			number
		});
		...
		for (let i = 0; i < duels.length; i++) this.createRound(stageId, groupId, i + 1, 1, [duels[i]]);
	}
	 ...
	 * @param positions An optional list of positions (seeds) for a manual ordering.
	 */
	private getSlotsUsingIds(seeding: Seeding, positions?: number[]): ParticipantSlot[] {
	...
	private insertStage(stage: OmitId<Stage>): number {
		...
		if (this.updateMode) existing = this.storage.select('stage', this.currentStageId);
	 ...
	 */
	private createConsolationFinal(stageId: number, losers: ParticipantSlot[][]): void {
	...
	private ensureSeedOrdering(stageId: number): void {
		...
		if (!this.storage.update('stage', stageId, stage)) throw Error('Could not update the stage.');
	}
}
