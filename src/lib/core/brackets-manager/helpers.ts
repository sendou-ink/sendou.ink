...
export function getMatchResult(match: MatchResults): Side | null {
	...
	if (match.opponent1?.result === 'win' || match.opponent2 === null) winner = 'opponent1';
...
export function setExtraFields(stored: MatchResults, match: DeepPartial<MatchResults>): void {
	function partialAssign(target: unknown, update: unknown, ignoredKeys: string[]): void {
...
export function getUpdatedMatchResults<T extends MatchResults>(
	...
	return {
		...
		...(enableByes
			? {
					opponent1:
						match.opponent1 === null ? null : { ...existing.opponent1, ...match.opponent1 },
					opponent2: match.opponent2 === null ? null : { ...existing.opponent2, ...match.opponent2 }
				}
			: {
					opponent1:
						match.opponent1 === null ? { id: null } : { ...existing.opponent1, ...match.opponent1 },
					opponent2:
						match.opponent2 === null ? { id: null } : { ...existing.opponent2, ...match.opponent2 }
				})
	};
}
