...
export class BaseGetter {
	 ...
	 */
	private getPreviousMatchesFinal(match: Match, stage: Stage, roundNumber: number): Match[] {
		if (stage.type === 'single_elimination')
	 ...
	 * @param stage The parent stage.
	 */
	private getPreviousMatchesFinalSingleElimination(match: Match, stage: Stage): Match[] {
	 ...
	 * @param roundNumber Number of the current round.
	 */
	private getPreviousMatchesFinalDoubleElimination(match: Match, roundNumber: number): Match[] {
		if (roundNumber > 1)
			// Double grand final
	 ...
	 * @param roundNumber Number of the round.
	 */
	private getPreviousMatchesLB(match: Match, stage: Stage, roundNumber: number): Match[] {
	 ...
	 * @param roundNumber Number of the round.
	 */
	private getMatchesBeforeMajorRound(match: Match, roundNumber: number): Match[] {
		return [
	...
	private getMatchesBeforeFirstRoundLB(
		...
		return [
			this.findMatch(winnerBracketId, roundNumberWB, helpers.getOriginPosition(match, 'opponent1')),
			this.findMatch(winnerBracketId, roundNumberWB, helpers.getOriginPosition(match, 'opponent2'))
		];
	}
	...
	private getNextMatchesWB(
		...
		return [
			...this.getNextMatchesUpperBracket(match, stage.type, roundNumber, roundCount),
	...
	private getNextMatchesLB(
		...
		if (roundNumber % 2 === 1) return this.getMatchAfterMajorRoundLB(match, roundNumber);
	 ...
	 */
	private getNextMatchesFinal(match: Match, roundNumber: number, roundCount: number): Match[] {
		if (
	 ...
	 * @param roundNumber The number of the current round.
	 */
	private getMatchAfterMajorRoundLB(match: Match, roundNumber: number): Match[] {
	 ...
	 * @param roundNumber The number of the current round.
	 */
	private getMatchAfterMinorRoundLB(match: Match, roundNumber: number): Match[] {
		return [this.getDiagonalMatch(match.group_id, roundNumber, match.number)];
	}
	 ...
	 * @param stageType The type of the stage.
	 */
	protected getSeedingMatches(stageId: number, stageType: StageType): Match[] | null {
		if (stageType === 'round_robin') return this.storage.select('match', { stage_id: stageId });
	 ...
	 * @param stageType Type of the stage.
	 */
	private getFinalGroupId(stageId: number, stageType: StageType): number | null {
		const groupNumber =
			stageType === 'single_elimination' ? 2 /* Consolation final */ : 3; /* Grand final */
	 ...
	 */
	private getDiagonalMatch(groupId: number, roundNumber: number, matchNumber: number): Match {
	 ...
	 * @param matchNumber Number of the match in its parent round.
	 */
	private getParallelMatch(groupId: number, roundNumber: number, matchNumber: number): Match {
	 ...
	 * @param matchNumber Number of the match in its parent round.
	 */
	protected findMatch(groupId: number, roundNumber: number, matchNumber: number): Match {
