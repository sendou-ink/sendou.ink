import type { GroupType, Match, Stage } from '$lib/core/brackets-model';
...
export class BaseUpdater extends BaseGetter {
	 ...
	 */
	protected updateRelatedMatches(match: Match, updatePrevious: boolean, updateNext: boolean): void {
		const { roundNumber, roundCount } = this.getRoundPositionalInfo(match.round_id);
		...
		if (updatePrevious) this.updatePrevious(match, matchLocation, stage, roundNumber);
		if (updateNext) this.updateNext(match, matchLocation, stage, roundNumber, roundCount);
	}
	 ...
	 */
	protected updateMatch(stored: Match, match: DeepPartial<Match>, force?: boolean): void {
	...
	protected applyMatchUpdate(match: Match): void {
		if (!this.storage.update('match', match.id, match)) throw Error('Could not update the match.');
	}
	...
	protected updatePrevious(
		...
		const previousMatches = this.getPreviousMatches(match, matchLocation, stage, roundNumber);
	...
	protected updateNext(
		...
		const nextMatches = this.getNextMatches(match, matchLocation, stage, roundNumber, roundCount);
