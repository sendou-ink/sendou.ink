...
// xxx: fix any TournamentMapListMap
type TournamentMapListMap = any;

export const types = ['COUNTERPICK', 'COUNTERPICK_MODE_REPEAT_OK', 'BAN_2'] as const;
...
export function turnOf({
	...
	switch (maps.pickBan) {
		case 'BAN_2': {
			...
			if (!mapList.some((map) => map.bannedByTournamentTeamId === firstPicker)) {
				return firstPicker;
			}

			if (!mapList.some((map) => map.bannedByTournamentTeamId === secondPicker)) {
				return secondPicker;
			}
		...
		case 'COUNTERPICK': {
			...
			if (isSetOverByResults({ count: maps.count, results, countType: maps.type })) {
				return null;
			}
			...
			const result = teams.find((tournamentTeamId) => latestWinner !== tournamentTeamId);
...
export function isLegal({ map, ...rest }: MapListWithStatusesArgs & { map: ModeWithStage }) {
	const pool = mapsListWithLegality(rest);

	return pool.some((m) => m.mode === map.mode && m.stageId === map.stageId && m.isLegal);
}
...
export function mapsListWithLegality(args: MapListWithStatusesArgs) {
			...
			case 'COUNTERPICK': {
				if (args.toSetMapPool.length === 0) {
					...
					for (const map of combinedPools) {
						if (!result.some((m) => m.mode === map.mode && m.stageId === map.stageId)) {
							result.push(map);
						}
					}
	...
	const unavailableModesSet =
		// one mode tournament
		unavailableModesSetAll.size < modesIncluded.length ? unavailableModesSetAll : new Set();

	const result = mapPool.map((map) => {
		const isLegal = !unavailableStagesSet.has(map.stageId) && !unavailableModesSet.has(map.mode);
...
function unavailableStages({
	...
	switch (maps.pickBan) {
		case 'BAN_2': {
			return new Set(
				mapList?.filter((m) => m.bannedByTournamentTeamId).map((map) => map.stageId) ?? []
			);
		}
		case 'COUNTERPICK_MODE_REPEAT_OK':
		case 'COUNTERPICK': {
...
function unavailableModes({
	...
	if (!maps?.pickBan || maps.pickBan === 'BAN_2' || maps.pickBan === 'COUNTERPICK_MODE_REPEAT_OK') {
		return new Set();
	}
