...
import { mapPoolToSerializedString } from './serializer';

export type MapPoolObject = Record<ModeShort, StageId[]>;

export class MapPool {
	private data: MapPoolObject;

	constructor(init: MapPoolObject) {
		this.data = init;
	}
	...
	get serialized() {
		return mapPoolToSerializedString(this.data);
	}
}
...
// 	type MapPoolObject,
// 	type ReadonlyMapPoolObject
// } from './serializer';

// export type DbMapPoolList = Array<Pick<Tables['MapPoolMap'], 'stageId' | 'mode'>>;
...
// 		for (const { stageId, mode } of init) {
...
// 		return (this.asSerialized =
// 			typeof this.source === 'string' ? this.source : mapPoolToSerializedString(this.source));
// 	}
...
// 		// biome-ignore lint/suspicious/noAssignInExpressions: biome migration
// 		return (this.asObject =
// 			typeof this.source === 'string' ? serializedStringToMapPool(this.source) : this.source);
// 	}
...
// 		return Object.values(this.parsed).some((stages) => stages.includes(stageId));
// 	}
