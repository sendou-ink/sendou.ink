// @ts-nocheck
// xxx: implement sets.server.ts file
...
export interface PlayedSet {
	...
	opponent: {
		...
		roster: Array<
			Pick<Tables['User'], 'id' | 'username' | 'discordAvatar' | 'discordId' | 'customUrl'>
		>;
	};
}
...
export function tournamentTeamSets({
	...
	return sets.map((set) => {
		const round = allRounds.find((round) => round.stageId === set.stageId) ?? allRounds[0];
		...
		function resolveRound() {
			...
			const maxRoundNumberOfGroup = Math.max(
				...allRounds
					.filter((round) => round.groupNumber === set.groupNumber && round.stageId === set.stageId)
...
function flipScoreIfNeeded(set: SetHistoryByTeamIdItem): [number, number] {
	const score: [number, number] = [set.opponentOneScore ?? 0, set.opponentTwoScore ?? 0];

	const wonTheSet =
		set.matches.reduce((acc, cur) => cur.wasWinner + acc, 0) > set.matches.length / 2;

	if ((wonTheSet && score[0] < score[1]) || (!wonTheSet && score[0] > score[1])) {
