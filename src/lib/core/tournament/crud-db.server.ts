...
export class Stage {
	...
	static getByTournamentId(tournamentId: number): StageType[] {
		return (stage_getByTournamentIdStm.all({ tournamentId }) as any[]).map(Stage.#convertStage);
	}
...
export class Group {
	...
	static getByStageId(stageId: Tables['TournamentStage']['id']): GroupType[] {
		return (group_getByStageIdStm.all({ stageId }) as any[]).map(Group.#convertGroup);
	}
...
export class Round {
	...
	static #convertRound(rawRound: Tables['TournamentRound'] & { maps?: string | null }): RoundType {
	...
	static getByStageId(stageId: Tables['TournamentStage']['id']): RoundType[] {
		return (round_getByStageIdStm.all({ stageId }) as any[]).map(Round.#convertRound);
	}

	static getByGroupId(groupId: Tables['TournamentGroup']['id']): RoundType[] {
		return (round_getByGroupIdStm.all({ groupId }) as any[]).map(Round.#convertRound);
	}
...
export class Match {
	...
	static getByRoundId(roundId: Tables['TournamentRound']['id']): MatchType[] {
		return (match_getByRoundIdStm.all({ roundId }) as any[]).map(Match.#convertMatch);
	}

	static getByStageId(stageId: Tables['TournamentStage']['id']): MatchType[] {
		return (match_getByStageIdStm.all({ stageId }) as any[]).map(Match.#convertMatch);
	}
