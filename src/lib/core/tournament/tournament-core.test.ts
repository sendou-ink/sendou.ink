...
describe('Follow-up bracket progression', () => {
	...
	test('includes correct teams in the top cut', () => {
		for (const tournamentTeamId of [892, 882, 881]) {
			expect(tournamentPP257.brackets[1].seeding?.some((team) => team === tournamentTeamId)).toBe(
				true
			);
		}
	});
	...
	test('underground bracket includes a checked in team', () => {
		expect(tournamentPP257.brackets[2].seeding?.some((team) => team === 902)).toBe(true);
	});
	...
	test('underground bracket includes all teams if does not require check in (DE->SE)', () => {
		expect(tournamentITZ32UndergroundWithoutCheckIn.brackets[1].seeding?.length).toBe(16);
	});

	test('underground bracket excludes checked out teams', () => {
		expect(tournamentITZ32UndergroundWithoutCheckInWithCheckedOut.brackets[1].seeding?.length).toBe(
			15
		);
	});
	...
	test('correct seed distribution in the top cut', () => {
		...
		for (const match of tournamentPP257.brackets[1].data.match) {
			const opponent1 = rrPlacements.find((placement) => placement.team.id === match.opponent1?.id);
			const opponent2 = rrPlacements.find((placement) => placement.team.id === match.opponent2?.id);
		...
		expect(ACTUAL_AMOUNT_OF_WORSE_VS_BEST, 'Amount of worse vs best is incorrect').toBe(
			AMOUNT_OF_WORSE_VS_BEST
		);
		expect(ACTUAL_AMOUNT_OF_WORSE_VS_WORSE, 'Amount of worse vs worse is incorrect').toBe(
			AMOUNT_OF_WORSE_VS_WORSE
		);
		expect(ACTUAL_AMOUNT_OF_BEST_VS_BEST, 'Amount of best vs best is incorrect').toBe(
			AMOUNT_OF_BEST_VS_BEST
		);
	});

	function validateNoRematches(rrMatches: Match[], topCutMatches: Match[]) {
		for (const topCutMatch of topCutMatches) {
			...
			for (const rrMatch of rrMatches) {
				if (
					...
					throw new Error(`Rematch detected: ${rrMatch.opponent1.id} vs ${rrMatch.opponent2.id}`);
				}
				if (
	...
	test('avoids rematches in RR -> SE (PP 255) - only minimum swap', () => {
		...
		for (const match of oldTopCutMatches) {
			...
			const newMatch = newTopCutMatches.find(
				(m) => m.opponent1?.id === match.opponent1.id && m.opponent2?.id === match.opponent2.id
			);
...
describe('Bracket progression override', () => {
	it('handles no override', () => {
		...
		expect(tournament.brackets[1].participantTournamentTeamIds).toHaveLength(11);
		expect(tournament.brackets[2].participantTournamentTeamIds).toHaveLength(11);
		expect(tournament.brackets[3].participantTournamentTeamIds).toHaveLength(11);
		expect(tournament.brackets[4].participantTournamentTeamIds).toHaveLength(11);
	});
	...
	it('overrides causing the team to go to another bracket', () => {
		...
		expect(tournament.brackets[1].participantTournamentTeamIds.includes(14809)).toBeTruthy();
	});
	...
	it('overrides causing the team not to go to their original bracket', () => {
		...
		expect(tournament.brackets[2].participantTournamentTeamIds.includes(14809)).toBeFalsy();
	});
	...
	it('destinationBracketIdx = -1 eliminates the team', () => {
		...
		expect(tournament.brackets[1].participantTournamentTeamIds).toHaveLength(11);
		expect(tournament.brackets[2].participantTournamentTeamIds).toHaveLength(10);
		expect(tournament.brackets[3].participantTournamentTeamIds).toHaveLength(11);
		expect(tournament.brackets[4].participantTournamentTeamIds).toHaveLength(11);
	});
	...
	it('if redundant override, still in the right bracket', () => {
		...
		expect(tournament.brackets[2].participantTournamentTeamIds.includes(14809)).toBeTruthy();
	});
	...
	it('redundants override does not affect the seed', () => {
		...
		const seedingIdx = tournament.brackets[2].seeding?.indexOf(tournamentTeamId);
		const seedingIdxWOverride = tournamentWOverride.brackets[2].seeding?.indexOf(tournamentTeamId);
...
describe('Adjusting team starting bracket', () => {
	function createTournament(teamStartingBracketIdx: (number | null)[]) {
