...
import type { Tables, TournamentStage, TournamentStageSettings } from '$lib/server/db/tables';
import {
	getTournamentManager,
	modesIncluded,
	tournamentIsRanked
} from '$lib/core/tournament/utils';
import {
	fillWithNullTillPowerOfTwo,
	groupNumberToLetters
} from '$lib/core/tournament-bracket/utils';
import type { Match, Stage } from '$lib/core/brackets-model';
...
export class Tournament {
	...
	constructor({
		...
		const teamsInSeedOrder = ctx.teams.sort((a, b) => {
			...
			return this.compareUnseededTeams(a, b, ctx.settings.minMembersPerTeam ?? 4);
		});
	...
	private initBrackets(data: TournamentManagerDataSet) {
		for (const [
			bracketIdx,
			{ type, name, sources, requiresCheckIn = false, startTime = null, settings }
			...
			if (inProgressStage) {
				...
				this.brackets.push(
					Bracket.create({
						...
						data: {
							...data,
							group: data.group.filter((group) => group.stage_id === inProgressStage.id),
							match,
							stage: data.stage.filter((stage) => stage.id === inProgressStage.id),
							round: data.round.filter((round) => round.stage_id === inProgressStage.id)
						},
						type
					})
				);
				...
				this.brackets.push(
					Bracket.create({
						...
						// 	name,
						// 	seeding: checkedInTeams,
						// 	settings: this.bracketManagerSettings(settings, type, checkedInTeams.length)
						// }),
						type,
						sources,
						createdAt: null,
						canBeStarted:
							(!startTime || isPast(startTime)) &&
				...
				const checkedInTeamsWithReplaysAvoided = this.avoidReplaysOfPreviousBracketOpponent(
					checkedInTeams,
					{
						sources,
						type
				...
				this.brackets.push(
					Bracket.create({
						...
						canBeStarted:
							(!startTime || isPast(startTime)) &&
							checkedInTeamsWithReplaysAvoided.length >= TOURNAMENT.ENOUGH_TEAMS_TO_START &&
							(sources ? relevantMatchesFinished : this.regularCheckInHasEnded),
						teamsPendingCheckIn: bracketIdx !== 0 ? notCheckedInTeams : undefined
					})
				);
	...
	private resolveTeamsFromSources(
		...
		for (const source of sources) {
			...
			const { teams: sourcedTeams, relevantMatchesFinished } = sourceBracket.source(
				source.placements
			);
		...
		const overridesWithoutRepeats = teamsFromOverride
			...
			.sort((a, b) => {
				...
				const aStanding = bracket.standings.find((standing) => standing.team.id === a.id);
				const bStanding = bracket.standings.find((standing) => standing.team.id === b.id);
	...
	private resolveTeamsFromSignups(bracketIdx: number) {
		const teams = this.isMultiStartingBracket
			? this.ctx.teams.filter((team) => {
					...
					const startingBracket = this.ctx.settings.bracketProgression.at(team.startingBracketIdx);
					if (!startingBracket || startingBracket.sources) {
						logger.warn('resolveTeamsFromSignups: Starting bracket index invalid');
						return bracketIdx === 0;
					}
	...
	private avoidReplaysOfPreviousBracketOpponent(
		...
		if (!sourceBracket) {
			logger.warn('avoidReplaysOfPreviousBracketOpponent: Source bracket not found');
			return teams;
		}
		...
		const bracketReplays = (candidateTeams: number[]) => {
			...
			manager.create({
				tournamentId: this.ctx.id,
				name: 'X',
				type: bracket.type as Exclude<TournamentStage['type'], 'round_robin' | 'swiss'>,
				seeding: fillWithNullTillPowerOfTwo(candidateTeams),
				settings: this.bracketManagerSettings(settings, bracket.type, candidateTeams.length)
			});
			...
			for (const match of matches) {
				...
				if (sourceBracketEncounters.get(match.opponent1.id)?.includes(match.opponent2.id)) {
		...
		while (replays.length > 0) {
			...
			const lowerSeedId =
				newOrder.findIndex((t) => t === oneId) < newOrder.findIndex((t) => t === twoId)
					? twoId
					: oneId;
	...
	bracketManagerSettings(
		...
		switch (type) {
			case 'single_elimination': {
				...
				return {
					consolationFinal:
						selectedSettings?.thirdPlaceMatch ?? TOURNAMENT.SE_DEFAULT_HAS_THIRD_PLACE_MATCH
				};
			}
			...
			case 'round_robin': {
				const teamsPerGroup =
					selectedSettings?.teamsPerGroup ?? TOURNAMENT.RR_DEFAULT_TEAM_COUNT_PER_GROUP;
	...
	resolvePoolCode({
		...
		const tournamentNameWithoutOnlyLetters = this.ctx.name.replace(/[^a-zA-Z ]/g, '');
		...
		if (!prefix) {
			prefix = ['AB', 'CD', 'EF', 'GH', 'IJ', 'KL', 'MN', 'OP', 'QR', 'ST'][this.ctx.id % 10];
		}
		...
		return {
			prefix,
			suffix: globalSuffix ?? groupLetters ?? bracketNumber ?? hostingTeamId % 10
		};
	}
	...
	participatedPlayersByTeamId(id: number) {
		...
		return team.members.filter((member) => this.ctx.participatedUsers.includes(member.userId));
	}
	...
	canFinalize(user: OptionalIdObject) {
		...
		const relevantBrackets = this.brackets.filter((b) => !b.preview || !b.isUnderground);
		...
		const everyRoundHasMatches = () => {
			...
			return this.brackets[0].data.round.every((round) => {
				const hasMatches = this.brackets[0].data.match.some((match) => match.round_id === round.id);
	...
	canReportScore({ matchId, user }: { matchId: number; user: OptionalIdObject }) {
		...
		const matchIsOver = match.opponent1.result === 'win' || match.opponent2.result === 'win';
		...
		const isParticipant =
			match.opponent1.id === teamMemberOf || match.opponent2.id === teamMemberOf;
	...
	get canAddNewSubPost() {
		...
		return (
			...
			this.ctx.settings.regClosesAt === dateToDatabaseTimestamp(this.ctx.startTime) ||
	...
	get regularCheckInIsOpen() {
		return this.regularCheckInStartsAt < new Date() && this.regularCheckInEndsAt > new Date();
	}
	...
	matchContextNamesById(matchId: number) {
			...
			for (const match of bracket.data.match) {
				if (match.id === matchId) {
					...
					if (bracket.type === 'round_robin') {
						const group = bracket.data.group.find((group) => group.id === match.group_id);
						const round = bracket.data.round.find((round) => round.id === match.round_id);
		...
		function roundNameWithoutMatchIdentifier(roundName?: string) {
			if (!roundName) return;
		...
		return {
			bracketName,
			roundName,
			roundNameWithoutMatchIdentifier: roundNameWithoutMatchIdentifier(roundName)
		};
	}
	...
	teamMemberOfProgressStatus(user: OptionalIdObject) {
		...
		if (this.brackets.every((bracket) => bracket.preview) && !this.regularCheckInIsOpen) {
			return null;
		}
		...
		for (const bracket of this.brackets) {
			if (bracket.preview) continue;
			for (const match of bracket.data.match) {
				const isParticipant = match.opponent1?.id === team.id || match.opponent2?.id === team.id;
				const isNotFinished =
					match.opponent1 &&
					match.opponent2 &&
				...
				if (isParticipant && isNotFinished && !isWaitingForTeam) {
					const otherTeam = this.teamById(
						match.opponent1!.id === team.id ? match.opponent2!.id! : match.opponent1!.id!
					)!;
					...
					const otherTeamBusyWithPreviousMatch =
						...
						bracket.data.match.find(
							(match) =>
								(match.opponent1?.id === otherTeam.id || match.opponent2?.id === otherTeam.id) &&
								match.opponent1?.result !== 'win' &&
								match.opponent2?.result !== 'win'
						)?.id !== match.id;
		...
		if (team.checkIns.length === 0 && this.regularCheckInIsOpen) {
			return {
				type: 'CHECKIN',
				canCheckIn: this.checkInConditionsFulfilledByTeamId(team.id).isFulfilled
			} as const;
		}
		...
		for (const bracket of this.brackets) {
			...
			const isParticipant = bracket.participantTournamentTeamIds.includes(team.id);

			const setsGeneratedCount = bracket.data.match.filter(
				(match) => match.opponent1?.id === team.id || match.opponent2?.id === team.id
			).length;
	...
	private matchAffectsAnotherBracket({
		...
		const participantInAnotherBracket = ongoingFollowUpBrackets
			...
			.some(
				(tournamentTeamId) =>
					tournamentTeamId === match.opponent1?.id || tournamentTeamId === match.opponent2?.id
			);
	...
	followingMatches(matchId: number) {
		...
		return bracket.data.match
			.filter(
				...
				(match2) => match2.stage_id === match.stage_id && match2.id !== match.id
			)
			.filter((match2) => {
				...
				const comesAfter = match2.group_id > match.group_id || match2.round_id > match.round_id;
	...
	isOrganizer(user: OptionalIdObject) {
		...
		if (
			this.ctx.organization?.members.some(
				(member) => member.userId === user.id && ['ADMIN', 'ORGANIZER'].includes(member.role)
			)
		) {
			return true;
		}

		return this.ctx.staff.some((staff) => staff.id === user.id && staff.role === 'ORGANIZER');
	}
	...
	isOrganizerOrStreamer(user: OptionalIdObject) {
		...
		if (
			this.ctx.organization?.members.some(
				(member) =>
					member.userId === user.id && ['ADMIN', 'ORGANIZER', 'STREAMER'].includes(member.role)
			)
		) {
			return true;
		}
		...
		return this.ctx.staff.some(
			(staff) => staff.id === user.id && ['ORGANIZER', 'STREAMER'].includes(staff.role)
		);
	}
}
