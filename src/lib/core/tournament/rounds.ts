...
export function getEliminationBracketRounds(args: {
	...
	const rounds = args.bracketData.round
		.flatMap((round) => {
			if (typeof round.group_id === 'number' && !groupIds.includes(round.group_id)) {
				return [];
			}
		...
		.filter((round, i, rounds) => {
			...
			const grandFinalsMatch =
				args.type === 'winners'
					? args.bracketData.match.find((match) => match.round_id === rounds[rounds.length - 2]?.id)
					: undefined;
			...
			const matches = args.bracketData.match.filter((match) => match.round_id === round.id);

			const atLeastOneNonByeMatch = matches.some((m) => m.opponent1 && m.opponent2);

			return atLeastOneNonByeMatch;
		});

	const hasThirdPlaceMatch =
		args.type === 'single' && R.unique(args.bracketData.match.map((m) => m.group_id)).length > 1;
	const namedRounds = rounds.map((round, i) => {
		function name() {
			if (showingBracketReset && args.type === 'winners' && i === rounds.length - 2) {
			...
			const namePrefix = args.type === 'winners' ? 'WB ' : args.type === 'losers' ? 'LB ' : '';

			function finalsOffSet() {
