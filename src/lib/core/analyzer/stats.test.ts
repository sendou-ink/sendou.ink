...
import { damageTypeToWeaponType } from '$lib/constants/analyzer';
...
describe('Analyze build', () => {
	test('Every main weapon has damage', () => {
		...
		for (const weaponSplId of mainWeaponIds) {
			...
			const hasDamage =
				analyzed.stats.damages.filter((dmg) => damageTypeToWeaponType[dmg.type] === 'MAIN').length >
				0;
	...
	test('Ninja Squid decreases swim speed', () => {
		...
		expect(analyzed.stats.swimSpeed.value).toBeGreaterThan(analyzedWithNS.stats.swimSpeed.value);
	});
	...
	test('Accounts for Jr. big ink tank with sub weapon ink consumption %', () => {
		...
		expect(analyzedDualieSquelchers.stats.subWeaponInkConsumptionPercentage.value).toBeGreaterThan(
