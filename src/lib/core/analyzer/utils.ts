...
import { UNKNOWN_SHORT } from '$lib/constants/analyzer';
...
export function isStackableAbility(ability: AbilityWithUnknown): ability is Ability {
...
export function isMainOnlyAbility(ability: AbilityWithUnknown): ability is Ability {
	if (ability === 'UNKNOWN') return false;
...
export function abilityValues({
	...
	return [overwrites?.High ?? High, overwrites?.Mid ?? Mid, overwrites?.Low ?? Low];
}
...
export function validatedAnyWeaponFromSearchParams(searchParams: URLSearchParams): AnyWeapon {
	...
	if (rawWeapon?.startsWith('SUB_')) {
		...
		if (!subWeaponIds.filter((id) => !nonBombSubWeaponIds.includes(id)).includes(id as any)) {
...
export function validatedWeaponIdFromSearchParams(searchParams: URLSearchParams) {
	const weaponId = searchParams.get('weapon') ? Number(searchParams.get('weapon')) : null;
...
function validateAbility(legalTypes: Array<AbilityType>, ability?: string): AbilityWithUnknown {
	if (!ability) throw new Error('Ability missing');
	if (ability === UNKNOWN_SHORT) return 'UNKNOWN';

	const abilityObj = abilities.find((a) => a.name === ability && legalTypes.includes(a.type));
...
export function validatedBuildFromSearchParams(
	...
	const abilitiesArr = searchParams.get(key) ? searchParams.get(key)?.split(',') : null;
