...
export function getAbilityChunksMapAsArray(build: BuildAbilitiesTupleWithUnknown) {
	...
	return Array.from(abilityChunksMap).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
}
		...
		for (const [index, selectedAbility] of gear.entries()) {
			...
			if (index === 0) {
				const primarySlotOnlyAbilityRef = abilities.filter(
					(a) => a.name === selectedAbility && a.abilityChunkTypesRequired.length > 0
				);
				...
				if (primarySlotOnlyAbilityRef.length === 1) {
					...
					for (const ability of primaryAbility.abilityChunkTypesRequired) {
						abilityChunksMap.set(
							ability,
							(abilityChunksMap.get(ability) ?? 0) + PRIMARY_SLOT_ONLY_REQUIRED_ABILITY_CHUNKS_COUNT
						);
					}
				} else {
					abilityChunksMap.set(
						selectedAbility,
						(abilityChunksMap.get(selectedAbility) ?? 0) + MAIN_REQUIRED_ABILITY_CHUNKS_COUNT
					);
				}
			}
