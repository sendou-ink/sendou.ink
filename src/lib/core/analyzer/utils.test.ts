...
describe('buildToAbilityPoints', () => {
	...
	test('calculates ability points correctly', () => {
		const aps = buildToAbilityPoints([['SS', 'SS', 'RSU', 'RSU'], EMPTY_ROW, EMPTY_ROW]);
	...
	test('handles ability doubler correctly', () => {
		const aps = buildToAbilityPoints([EMPTY_ROW, ['AD', 'SS', 'UNKNOWN', 'UNKNOWN'], EMPTY_ROW]);

		expect(aps.get('SS')).toBe(6);
	});

	test('does not calculate AP for main only abilities', () => {
		const aps = buildToAbilityPoints([['LDE', 'SS', 'RSU', 'RSU'], EMPTY_ROW, EMPTY_ROW]);
