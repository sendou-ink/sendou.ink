...
describe('getAbilityChunksMapAsArray()', () => {
	test('Empty build results in an empty array', () => {
		...
		expect(abilityChunksArray, 'Ability chunks array is not empty.').toHaveLength(0);
	});
	...
	describe('getAbilityChunksMapAsArray()', () => {
		...
		test('Ability Doubler ability does not count towards Ability Chunks', () => {
			...
			const abilityChunksArray = getAbilityChunksMapAsArray(buildWithOnlyAbilityDoubler);
