...
import type { Ability, MainWeaponId, SubWeaponId } from '$lib/constants/in-game/types';
import {
	INK_CONSUME_TYPES,
	type AbilityPoints,
	type AnalyzedBuild,
	type DamageType,
	type InkConsumeType,
	type MainWeaponParams,
	type SpecialWeaponParams,
	type StatFunctionInput,
	type SubWeaponParams
} from './types';
import invariant from '$lib/utils/invariant';
import { cutToNDecimalPlaces, roundToNDecimalPlaces } from '$lib/utils/number';
import { assertUnreachable } from '$lib/utils/types';
import { DAMAGE_TYPE, RAINMAKER_SPEED_PENALTY_MODIFIER } from '$lib/constants/analyzer';
import {
	ANGLE_SHOOTER_ID,
	AUTO_BOMB_ID,
	BURST_BOMB_ID,
	...
	TORPEDO_ID,
	ZIPCASTER_ID
} from '$lib/constants/in-game/weapon-ids';
...
export function buildStats({
	...
	const subWeaponParams = weaponParams().subWeapons[mainWeaponParams.subWeaponId];
	invariant(subWeaponParams, `Sub weapon with splId ${mainWeaponParams.subWeaponId} not found`);

	const specialWeaponParams = weaponParams().specialWeapons[mainWeaponParams.specialWeaponId];
	invariant(
		specialWeaponParams,
	...
	return {
		weapon: {
			...
			brellaCanopyHp: mainWeaponParams.CanopyHP && mainWeaponParams.CanopyHP / 10,
		...
		stats: {
			...
			subWeaponInkConsumptionPercentage: subWeaponInkConsumptionPercentage(input),
			...
			shootingRunSpeedFullCharge: shootingRunSpeed(input, 'MoveSpeedFullCharge'),
			shootingRunSpeedSecondaryMode: shootingRunSpeed(input, 'MoveSpeedVariable'),
			...
			framesBeforeTakingDamageInEnemyInk: framesBeforeTakingDamageInEnemyInk(input),
			...
			subDefPointSensorMarkedTimeInSeconds: subDefPointSensorMarkedTimeInSeconds(input),
			subDefInkMineMarkedTimeInSeconds: subDefInkMineMarkedTimeInSeconds(input),
			subDefAngleShooterMarkedTimeInSeconds: subDefAngleShooterMarkedTimeInSeconds(input),
			...
			specialHookInkConsumptionPercentage: specialHookInkConsumptionPercentage(input),
			specialInkConsumptionPerSecondPercentage: specialInkConsumptionPerSecondPercentage(input),
...
function specialLost(
	...
	const splattedByExtraPenalty = splattedByRP ? ENEMY_RESPAWN_PUNISHER_EXTRA_SPECIAL_LOST : 0;

	return {
		baseValue: specialSavedAfterDeathForDisplay(baseEffect - splattedByExtraPenalty),
		value: specialSavedAfterDeathForDisplay(effect - splattedByExtraPenalty - extraSpecialLost),
...
function subWeaponInkConsumptionPercentage(args: StatFunctionInput) {
	return {
		...
		value: roundToNDecimalPlaces(
			...
			(subWeaponConsume(args).inkConsume * 100 + 0.0045) / inkTankSize(args.weaponSplId)
		)
	};
}
...
export function fullInkTankOptions(
	...
	const { inkConsume: subWeaponInkConsume, maxSubsFromFullInkTank } = subWeaponConsume(args);
		...
		for (const type of INK_CONSUME_TYPES) {
			...
			result.push({
				...
				value: effectToRounded(
					(inkTankSize(args.weaponSplId) - subWeaponInkConsume * subsFromFullInkTank) /
...
function subWeaponConsume({
	...
	return {
		...
		maxSubsFromFullInkTank: Math.floor(inkTankSize(weaponSplId) / inkConsumeAfterISS)
	};
}
...
function inkConsumeTypeToParamsKeys(type: InkConsumeType): Array<keyof MainWeaponParams> {
	switch (type) {
		case 'NORMAL':
...
const damageTypeToParamsKey: Record<
	...
	| Array<keyof MainWeaponParams | keyof SubWeaponParams | keyof SpecialWeaponParams>
> = {
				...
				if (Array.isArray(value)) {
					...
					if (id === BURST_BOMB_ID) {
						arrayValues.unshift({
							...
							value: cutToNDecimalPlaces(R.sum(arrayValues.map((v) => v.value)), 1),
							type
						});
					}
					...
					if (id === FIZZY_BOMB_ID || id === CURLING_BOMB_ID) {
						const allArrayValues = arrayValues.sort((a, b) => a.baseValue - b.baseValue);
						const firstHalfValues = allArrayValues.slice(0, allArrayValues.length / 2);
						const secondHalfValues = allArrayValues.slice(allArrayValues.length / 2);
						...
						arrayValues = [
							{
								...
								subWeaponId: id,
								distance: [
									Math.min(...secondHalfValues.map((value) => value.distance as number)),
									Math.max(...secondHalfValues.map((value) => value.distance as number))
								],
							...
							},
							{
								...
								subWeaponId: id,
								distance: [
									Math.min(...firstHalfValues.map((value) => value.distance as number)),
									Math.max(...firstHalfValues.map((value) => value.distance as number))
								],
...
function subWeaponIdToEffectKey(subWeaponId: SubWeaponId): keyof typeof abilityValuesJson {
	switch (subWeaponId) {
		case SPLAT_BOMB_ID:
		...
		default:
			throw new Error(`No damage rate for the sub weapon with id: ${subWeaponId}`);
	}
}
...
function runSpeedInEnemyInk(args: StatFunctionInput): AnalyzedBuild['stats']['runSpeedInEnemyInk'] {
...
function shootingRunSpeed(
	args: StatFunctionInput,
	keyName: 'MoveSpeed' | 'MoveSpeed_Charge' | 'MoveSpeedFullCharge' | 'MoveSpeedVariable'
...
function swimSpeed(args: StatFunctionInput): AnalyzedBuild['stats']['swimSpeed'] {
	const key =
		args.mainWeaponParams.WeaponSpeedType === 'Fast'
			? '_Fast'
			: args.mainWeaponParams.WeaponSpeedType === 'Slow'
...
function swimSpeedHoldingRainmaker(
	...
	return {
		...withoutRM,
		baseValue: effectToRounded(withoutRM.baseValue * RAINMAKER_SPEED_PENALTY_MODIFIER),
...
function respawnTime(
	...
	const ownRPExtraFrames = hasRespawnPunisher ? OWN_RESPAWN_PUNISHER_EXTRA_RESPAWN_FRAMES : 0;

	const splattedByExtraFrames = splattedByRP ? ENEMY_RESPAWN_PUNISHER_EXTRA_RESPAWN_FRAMES : 0;
...
function superJumpTimeTotal(args: StatFunctionInput): AnalyzedBuild['stats']['superJumpTimeTotal'] {
	...
	return {
		baseValue: framesToSeconds(Math.ceil(charge.baseEffect) + Math.ceil(move.baseEffect)),
		value: framesToSeconds(Math.ceil(charge.effect) + Math.ceil(move.effect)),
		modifiedBy: SUPER_JUMP_TIME_TOTAL_ABILITY
	};
}

function shotSpreadAir(args: StatFunctionInput): AnalyzedBuild['stats']['shotSpreadAir'] {
...
export function subStats(args: Pick<StatFunctionInput, 'subWeaponParams' | 'abilityPoints'>) {
...
function subQsjBoost(args: StatFunctionInput): AnalyzedBuild['stats']['subQsjBoost'] {
	if (
		!hasEffect({
			key: 'SubSpecUpParam',
			weapon: args.subWeaponParams
...
function specialPaintRadius(args: StatFunctionInput): AnalyzedBuild['stats']['specialPaintRadius'] {
...
function specialMoveSpeed(args: StatFunctionInput): AnalyzedBuild['stats']['specialMoveSpeed'] {
	if (
		!hasEffect({
			key: 'MoveSpeed',
			weapon: args.specialWeaponParams
...
function specialMaxRadius(args: StatFunctionInput): AnalyzedBuild['stats']['specialMaxRadius'] {
	if (
		!hasEffect({
			key: 'MaxRadius',
			weapon: args.specialWeaponParams
